...
static struct ext4_xattr_header* xattr_list_find(struct ext4_inode* inode) {
    ...
    for (element = aux_info.xattrs; element != NULL; element = element->next) {
        if (element->inode == inode) return element->header;
    }
    return NULL;
}
...
static void region_list_remove(struct region_list* list, struct region* reg) {
    ...
    if (list->first == reg) list->first = reg->next;

    if (list->last == reg) list->last = reg->prev;
...
void append_region(struct block_allocation* alloc, u32 block, u32 len, int bg_num) {
    struct region* reg;
...
static void allocate_bg_inode_table(struct block_group_info* bg) {
    if (bg->inode_table != NULL) return;

    u32 block = bg->first_block + 2;

    if (bg->has_superblock) block += aux_info.bg_desc_blocks + info.bg_desc_reserve_blocks + 1;

    bg->inode_table = calloc(aux_info.inode_table_blocks, info.block_size);
    if (bg->inode_table == NULL) critical_error_errno("calloc");
...
static int reserve_blocks(struct block_group_info* bg, u32 start, u32 num) {
    ...
    u32 block = start;
    if (num > bg->free_blocks) return -1;
    ...
    bg->free_blocks -= num;
    if (start == bg->first_free_block) bg->first_free_block = start + num;
...
static void free_blocks(struct block_group_info* bg, u32 num_blocks) {
    ...
    for (i = 0; i < num_blocks; i++, block--) bg->block_bitmap[block / 8] &= ~(1 << (block % 8));
...
static void init_bg(struct block_group_info* bg, unsigned int i) {
    ...
    if (bg->has_superblock) block += 1 + aux_info.bg_desc_blocks + info.bg_desc_reserve_blocks;
    sparse_file_add_data(ext4_sparse_file, bg->bitmaps, 2 * info.block_size, block);
...
void block_allocator_init() {
    ...
    if (aux_info.bgs == NULL) critical_error_errno("calloc");

    for (i = 0; i < aux_info.groups; i++) init_bg(&aux_info.bgs[i], i);
}
...
static u32 ext4_allocate_blocks_from_block_group(u32 len, int bg_num) {
    if (get_free_blocks(bg_num) < len) return EXT4_ALLOCATE_FAILED;
...
u32 allocate_block() {
    ...
    for (i = 0; i < aux_info.groups; i++) {
        ...
        if (block != EXT4_ALLOCATE_FAILED) return block;
    }
...
static struct region* ext4_allocate_best_fit(u32 len) {
    ...
    while (len > 0) {
        ...
        if (first_reg == NULL) first_reg = reg;
...
/* Allocate len blocks.  The blocks may be spread across multiple block groups,
   ...
   allocation algorithm is:
      1.  If the remaining allocation is larger than any available contiguous
   region, allocate the largest contiguous region and loop
...
int block_allocation_num_regions(struct block_allocation* alloc) {
    ...
    for (i = 0; reg != NULL; reg = reg->next) i++;
...
int block_allocation_len(struct block_allocation* alloc) {
    ...
    for (i = 0; reg != NULL; reg = reg->next) i += reg->len;
...
u32 get_block(struct block_allocation* alloc, u32 block) {
    ...
    for (; reg; reg = reg->next) {
        if (block < reg->len) return reg->block + block;
        block -= reg->len;
    }
    return EXT4_ALLOCATE_FAILED;
}
...
int append_oob_allocation(struct block_allocation* alloc, u32 len) {
    ...
    for (; reg; reg = reg->next) region_list_append(&alloc->oob_list, reg);
...
struct ext4_inode* get_inode(u32 inode) {
    ...
    return (struct ext4_inode*)(aux_info.bgs[bg].inode_table + inode * info.inode_size);
}
...
u32 reserve_inodes(int bg, u32 num) {
    ...
    if (get_free_inodes(bg) < num) return EXT4_ALLOCATE_FAILED;
...
u32 allocate_inode(void) {
    ...
    for (bg = 0; bg < aux_info.groups; bg++) {
        ...
        if (inode != EXT4_ALLOCATE_FAILED) return bg * info.inodes_per_group + inode;
    }
