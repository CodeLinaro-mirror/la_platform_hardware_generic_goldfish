...
static int scandir_win(const char* path, struct dirent*** namelist) {
    ...
    int nameBufferSizeW = MultiByteToWideChar(CP_UTF8, 0, nameBuffer, nameBufferSize, NULL, 0);
    wchar_t* nameBufferW = calloc(nameBufferSizeW, sizeof(wchar_t));
    MultiByteToWideChar(CP_UTF8, 0, nameBuffer, nameBufferSize, nameBufferW, nameBufferSizeW);
    HANDLE hndl;
    ...
    *namelist = malloc(capacity * sizeof(struct dirent*));
    int idx = 0;
    do {
        if (wcscmp(L".", fileData.cFileName) != 0 && wcscmp(L"..", fileData.cFileName) != 0) {
            if (idx >= capacity) {
                capacity *= 2;
                *namelist = realloc(*namelist, capacity * sizeof(struct dirent*));
            }
            (*namelist)[idx] = malloc(sizeof(struct dirent));
            assert(wcslen(fileData.cFileName) < MAX_PATH);
            WideCharToMultiByte(CP_UTF8, 0, fileData.cFileName, -1, (*namelist)[idx]->d_name,
                                MAX_PATH, NULL, NULL);
            idx++;
        }
...
static u32 build_directory_structure(const char* full_path, const char* dir_path, u32 dir_inode,
    ...
    if (dir_inode == 0) {
        ...
        for (i = 0; i < entries; i++)
            if (strcmp(namelist[i]->d_name, "lost+found") == 0) break;
        if (i == entries) needs_lost_and_found = true;
    }
#endif

    dentries = calloc(entries, sizeof(struct dentry));
    if (dentries == NULL) critical_error_errno("malloc");
...
#else
        dentries[i].mode =
                _stat.st_mode & (S_ISUID | S_ISGID | S_ISVTX | S_IRWXU | S_IRWXG | S_IRWXO);
#endif
    ...
    for (i = 0; i < entries; i++) {
        ...
        *dentries[i].inode = entry_inode;
        ...
        ret = inode_set_permissions(entry_inode, dentries[i].mode, dentries[i].uid, dentries[i].gid,
                                    dentries[i].mtime);
        if (ret) error("failed to set permissions on %s\n", dentries[i].path);
        ...
        if (ret) error("failed to set SELinux context on %s\n", dentries[i].path);
        ret = inode_set_capabilities(entry_inode, dentries[i].capabilities);
        if (ret) error("failed to set capability on %s\n", dentries[i].path);
...
static u32 compute_bg_desc_reserve_blocks() {
    ...
    u32 block_groups = DIV_ROUND_UP(blocks, info.blocks_per_group);
    u32 bg_desc_blocks =
            DIV_ROUND_UP(block_groups * sizeof(struct ext2_group_desc), info.block_size);

    u32 bg_desc_reserve_blocks =
            DIV_ROUND_UP(block_groups * 1024 * sizeof(struct ext2_group_desc), info.block_size) -
            bg_desc_blocks;
...
int make_ext4fs_internal(int fd, const char* _directory, const char* _mountpoint,
    ...
    if (info.len <= 0) info.len = get_file_size(fd);
    ...
    if (info.block_size <= 0) info.block_size = compute_block_size();

    /* Round down the filesystem length to be a multiple of the block size */
    info.len &= ~((u64)info.block_size - 1);

    if (info.journal_blocks == 0) info.journal_blocks = compute_journal_blocks();
    ...
    if (info.blocks_per_group <= 0) info.blocks_per_group = compute_blocks_per_group();

    if (info.inodes <= 0) info.inodes = compute_inodes();
    ...
    info.feat_compat |= EXT4_FEATURE_COMPAT_RESIZE_INODE | EXT4_FEATURE_COMPAT_EXT_ATTR;

    info.feat_ro_compat |= EXT4_FEATURE_RO_COMPAT_SPARSE_SUPER | EXT4_FEATURE_RO_COMPAT_LARGE_FILE |
    ...
    if (reserve_inodes(0, 10) == EXT4_ALLOCATE_FAILED) error("failed to reserve first 10 inodes");

    if (info.feat_compat & EXT4_FEATURE_COMPAT_HAS_JOURNAL) ext4_create_journal_inode();

    if (info.feat_compat & EXT4_FEATURE_COMPAT_RESIZE_INODE) ext4_create_resize_inode();
