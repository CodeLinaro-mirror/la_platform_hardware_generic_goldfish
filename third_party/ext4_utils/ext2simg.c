...
static int build_sparse_ext(int fd, const char* filename) {
    ...
    if (!block_bitmap) critical_error("failed to allocate block bitmap");
    ...
    for (i = 0; i < aux_info.groups; i++) {
        ...
        ret = lseek64(fd, (u64)info.block_size * aux_info.bg_desc[i].bg_block_bitmap, SEEK_SET);
        if (ret < 0) critical_error_errno("failed to seek to block group bitmap %d", i);

        ret = read(fd, block_bitmap, info.block_size);
        if (ret < 0) critical_error_errno("failed to read block group bitmap %d", i);
        ...
        for (block = 0; block < last_block; block++) {
            if (start_contiguous_block >= 0) {
                ...
                if (bitmap_get_bit(block_bitmap, block)) start_contiguous_block = block;
            }
        }
        ...
        if (start_contiguous_block >= 0) {
            ...
            sparse_file_add_file(ext4_sparse_file, filename, (u64)info.block_size * start_block,
                                 info.block_size * len_blocks, start_block);
        }
    }
...
int main(int argc, char** argv) {
    ...
    if (infd < 0) critical_error_errno("failed to open input image");
