...
int32_t WindowsInstaller::getVersion(const char* productDisplayName) {
    ...
    LONG result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, registry_path, 0, samDesired, &hkey);
    ...
    for (DWORD i = 0; i < cSubKeys; i++) {
        ...
        HKEY hkey = 0;
        result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, install_properties.c_str(), 0, samDesired, &hkey);
        if (result != ERROR_SUCCESS) {
        ...
        if (0 == strcmp(display_name8.c_str(), productDisplayName)) {
            ...
            result = RegGetValue(hInstallPropertiesKey.get(), nullptr, "Version",
