...
void* _android_array_alloc(size_t itemSize, size_t count) {
    ...
    if (itemSize == 0) AASSERT_FAIL("item size is 0\n");

    maxSize = (~(size_t)0) / itemSize;
    if (count > maxSize) AASSERT_FAIL("allocation too large (%d > %d)\n", count, maxSize);
#endif
...
#ifdef _WIN32
char* win32_strsep(char** pline, const char* delim) {
    char* line = *pline;
    char* p = line;
...
android_thread_id_t android_get_thread_id() {
#ifdef _WIN32
