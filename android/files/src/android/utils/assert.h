...
void _android_assert_loc(const char* fileName, long fileLineno, const char* functionName);
...
#define _ANDROID_ASSERT_LOC() _android_assert_loc(__FILE__, __LINE__, __FUNCTION__)

/* Report an assertion failure then panic. Arguments are formatted string */
#define _ANDROID_ASSERT_FAIL(...) android_assert_fail(__VA_ARGS__)
...
#define AASSERT_STATIC(cond) \
    typedef char _AASSERT_CONCAT(__android_assertion_failed_, __LINE__)[2 * !!(cond) - 1];
