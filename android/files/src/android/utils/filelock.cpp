...
static bool delete_file(const android::base::Win32UnicodeString& name) {
    ...
    return retry([&name]() {
        ...
        auto deleted = ::DeleteFileW(name.c_str()) != 0 || ::GetLastError() == ERROR_FILE_NOT_FOUND;
        if (!deleted) {
...
static bool delete_dir(const android::base::Win32UnicodeString& name) {
    return retry([&name]() {
        return ::RemoveDirectoryW(name.c_str()) != 0 || ::GetLastError() == ERROR_FILE_NOT_FOUND;
    });
}
...
static int filelock_lock(FileLock* lock, int timeout) {
    ...
    for (;;) {
        ...
        if (!::CreateDirectoryW(unicodeDir.c_str(), nullptr) &&
            ...
            LOG(ERROR) << "Unexpected error while creating: " << unicodeDir.toString()
                       << "error: " << ::GetLastError();
            continue;
        }
        ...
        lockHandle = ::CreateFileW(unicodeName.c_str(),
        ...
        if (lockHandle == INVALID_HANDLE_VALUE &&
            ...
            ScopedFileHandle getpidHandle(::CreateFileW(
                    ...
                    FILE_SHARE_READ | FILE_SHARE_WRITE,  // allow others to r+w the file
                ...
                if (::ReadFile(getpidHandle.get(), buf, pidCharsMax, &bytesRead, nullptr)) {
                    ...
                    if (sscanf(buf, "%lu", &lockingPid) == 1) {
                        ...
                        LOG(INFO) << "Waiting " << sleep_duration_ms
                                  << " ms for process with pid: " << lockingPid << " to exit.";
    ...
    auto fileDeleter =
            android::base::makeCustomScopedPtr(lockHandle, [&unicodeName, &unicodeDir](HANDLE h) {
                ::CloseHandle(h);
    ...
    DWORD bytesWritten;
    if (!::WriteFile(lockHandle, pidBuf, static_cast<DWORD>(len), &bytesWritten, nullptr) ||
        bytesWritten != static_cast<DWORD>(len)) {
        LOG(WARNING) << "Failed to write the current PID (" << pid << ") into the lock file";
        return -1;
    }
    lock->locked = 1;
...
static auto filelock_lock(FileLock* lock, int timeout) -> int {
    ...
    for (;;) {
        ...
        if (S_ISDIR(st_lock.st_mode)) {
            ...
            if (rc != 0) {
                D("Removing stale Win32 lockfile '%s' failed (%s)", lock->lock, strerror(errno));
            }
        ...
        if (freshness == FRESHNESS_UNKNOWN) {
            ...
            freshness = (now >= st.st_mtime + 60) ? FRESHNESS_STALE : FRESHNESS_FRESH;
        }
        ...
        sleep_duration_ms = std::min(timeout, sleep_duration_ms + kSleepDurationMsIncrement);
    }
