...
static __inline__ int ispathsep(int c) {
#ifdef _WIN32
    return (c == '/' || c == '\\');
#else
    return (c == '/');
...
char* path_parent(const char* path, int levels) {
    ...
    while (levels > 0) {
        ...
        while (end > path && ispathsep(end[-1])) end--;

        base = end;
        while (base > path && !ispathsep(base[-1])) base--;

        if (base <= path) {
            if (end == base + 1 && base[0] == '.' && levels == 1) return strdup("..");
            /* we can't go that far */
            return NULL;
        }

        if (end == base + 1 && base[0] == '.') goto Next;
 ...
 * (error code in errno) */
APosixStatus path_android_mkdir(const char* path, int mode) {
    return HANDLE_EINTR(android_mkdir(path, mode));
}

static APosixStatus path_mkdir_recursive(char* path, unsigned len, int mode) {
    char old_c;
    int ret;
    unsigned len2;

    /* get rid of trailing separators */
    while (len > 0 && ispathsep(path[len - 1])) len -= 1;
    ...
    len2 = len - 1;
    while (len2 > 0 && !ispathsep(path[len2 - 1])) len2 -= 1;
...
APosixStatus path_mkdir_if_needed(const char* path, int mode) {
    int ret = 0;
...
APosixStatus path_mkdir_if_needed_no_cow(const char* path, int mode) {
    ...
    if (!res) {
        // #ifdef __linux__
        auto proc = android::base::Command::create({
                                                           "chattr",
                                                           "+C",
                                                           path,
                                                   })
                            .execute();
        proc->wait_for(1000ms);
        // #endif
    }
...
APosixStatus path_get_size(const char* path, uint64_t* psize) {
#ifdef _WIN32
...
APosixStatus path_is_same(const char* left, const char* right, bool* isSame) {
    ...
    if (Win32UnicodeString::convertFromUtf8(wideBuffer, arraySize(wideBuffer), left) < 0) {
        return -1;
    }
    ...
    // directories.
    ScopedFileHandle leftHandle(
            CreateFileW(wideBuffer, 0, FILE_SHARE_DELETE | FILE_SHARE_WRITE | FILE_SHARE_READ,
    ...
    if (Win32UnicodeString::convertFromUtf8(wideBuffer, arraySize(wideBuffer), right) < 0) {
        return -1;
    }
    ScopedFileHandle rightHandle(
            CreateFileW(wideBuffer, 0, FILE_SHARE_DELETE | FILE_SHARE_WRITE | FILE_SHARE_READ,
                        nullptr, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, nullptr));
    ...
    *isSame = leftInfo.dwVolumeSerialNumber == rightInfo.dwVolumeSerialNumber &&
    ...
    *isSame = leftStat.st_dev == rightStat.st_dev && leftStat.st_ino == rightStat.st_ino;
    return 0;
#endif
}

APosixStatus path_empty_file(const char* path) {
#ifdef _WIN32
...
APosixStatus path_copy_file_impl(const char* dest, const char* source) {
    ...
    int fs, fd;
    {
        ...
        if (Win32UnicodeString::convertFromUtf8(wideSrc, arraySize(wideSrc), source) < 0) {
            return -1;
        }
        if (Win32UnicodeString::convertFromUtf8(wideDest, arraySize(wideDest), dest) < 0) {
            return -1;
        }
...
APosixStatus path_copy_file(const char* dest, const char* source) {
...
APosixStatus path_delete_file(const char* path) {
...
void* path_load_file(const char* fn, size_t* pSize) {
    char* data;
    int sz;
    int fd;
...
char* path_search_exec(const char* filename) {
...
char* path_escape_path(const char* src) {
    if (!src) return NULL;
...
void path_unescape_path(char* str) {
    char* src = str;
    char* dst = str;
...
APosixStatus path_copy_dir(const char* dst, const char* src) {
    ...
    auto dirScanner = android::base::makeCustomScopedPtr(dirScanner_new(src), dirScanner_free);
...
APosixStatus path_copy_dir_ex(const char* dst, const char* src, void* pSkipSet) {
    bool isSameDir = false;
    int status = path_is_same(src, dst, &isSameDir);
    if (status != 0 || isSameDir) {
        return status;
    ...
    while (baseName) {
        if (doNotCopySet.find(baseName) != doNotCopySet.end()) {
            ...
            std::string fullSrcName = PathUtils::join(src, baseName);
            if (path_is_dir(fullSrcName.c_str())) {
                if (path_copy_dir_ex(fullDstName.c_str(), fullSrcName.c_str(), pSkipSet) < 0) {
                    return -1;
                }
            } else {
                if (path_copy_file(fullDstName.c_str(), fullSrcName.c_str()) < 0) {
...
APosixStatus path_delete_dir(const char* path) {
    APosixStatus contentsDeleteStatus = path_delete_dir_contents(path);
...
APosixStatus path_delete_dir_contents(const char* path) {
    auto dirScanner = android::base::makeCustomScopedPtr(dirScanner_new(path), dirScanner_free);
    if (!dirScanner) return -EINVAL;
...
APosixStatus path_delete_file_on_reboot(const char* path) {
    ...
    Win32UnicodeString utf16Path(path);
    BOOL result = MoveFileExW(utf16Path.c_str(), NULL, MOVEFILE_DELAY_UNTIL_REBOOT);
