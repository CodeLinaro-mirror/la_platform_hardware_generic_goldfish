...
APosixStatus path_mkdir_if_needed_no_cow(const char *path, int mode) {
  ...
  if (!res) {
// #ifdef __linux__
    auto proc = android::base::Command::create({"chattr", "+C", path,}).execute();
...
APosixStatus path_get_size(const char *path, uint64_t *psize) {
  ...
  LARGE_INTEGER size;
  HANDLE file =
      CreateFileW(/* lpFilename */ utf16Path.c_str(),
...
APosixStatus path_is_same(const char *left, const char *right, bool *isSame) {
  ...
  // directories.
  ScopedFileHandle leftHandle(CreateFileW(
      wideBuffer, 0, FILE_SHARE_DELETE | FILE_SHARE_WRITE | FILE_SHARE_READ,
  ...
  ScopedFileHandle rightHandle(CreateFileW(
      wideBuffer, 0, FILE_SHARE_DELETE | FILE_SHARE_WRITE | FILE_SHARE_READ,
      nullptr, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, nullptr));
  ...
  *isSame = leftStat.st_dev == rightStat.st_dev &&
  ...
  if (fs >= 0 && fd >= 0) {
    ...
    while ((n = HANDLE_EINTR(read(fs, buf, sizeof(buf)))) != 0) {
      if (HANDLE_EINTR(write(fd, buf, n)) != n) {
        ...
        D("Failed to copy '%s' to '%s': %s (%d)", source, dest, strerror(errno),
          errno);
        result = -1;
        break;
      }
...
APosixStatus path_copy_dir(const char *dst, const char *src) {
  ...
  auto dirScanner =
      android::base::makeCustomScopedPtr(dirScanner_new(src), dirScanner_free);
  if (!dirScanner)
    return -1;
...
APosixStatus path_copy_dir_ex(const char *dst, const char *src,
                              void *pSkipSet) {
  bool isSameDir = false;
  int status = path_is_same(src, dst, &isSameDir);
  if (status != 0 || isSameDir) {
...
APosixStatus path_delete_dir_contents(const char *path) {
  auto dirScanner =
      android::base::makeCustomScopedPtr(dirScanner_new(path), dirScanner_free);
  if (!dirScanner)
    return -EINVAL;
...
APosixStatus path_delete_file_on_reboot(const char *path) {
  ...
  Win32UnicodeString utf16Path(path);
  BOOL result =
      MoveFileExW(utf16Path.c_str(), NULL, MOVEFILE_DELAY_UNTIL_REBOOT);
