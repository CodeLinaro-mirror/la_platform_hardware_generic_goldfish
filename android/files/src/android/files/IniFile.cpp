...
static bool isKeyChar(unsigned uc) {
    ...
    return isKeyStartChar(uc) || (uc - '0' < numRange) || (uc == '.') || (uc == '-');
}
...
bool IniFile::read(bool keepComments) {
    ...
    if (begPos == kInvalidPos || endPos == kInvalidPos || endPos - begPos > kMaxIniFileSize) {
        LOG(WARNING) << ".ini File " << mBackingFilePath << " too large (" << (endPos - begPos)
                     << " bytes)";
        return false;
    }
...
string IniFile::getString(const string& key, std::string_view defaultValue) const {
    auto citer = mData.find(key);
    return envSubst((citer == std::end(mData)) ? defaultValue : std::string_view(citer->second));
}
...
static IniFile::DiskSize parseDiskSize(std::string_view valueStr, IniFile::DiskSize defaultValue,
                                       bool* outMalformed) {
    if (outMalformed) {
        *outMalformed = false;
    }
...
IniFile::DiskSize IniFile::getDiskSize(const string& key, IniFile::DiskSize defaultValue) const {
    if (!hasKey(key)) {
        return defaultValue;
    }
...
IniFile::DiskSize IniFile::getDiskSize(const string& key, std::string_view defaultValue) const {
