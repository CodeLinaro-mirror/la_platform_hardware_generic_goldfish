...
bool statEqual(struct stat lhs, struct stat rhs) {
    return lhs.st_dev == rhs.st_dev && lhs.st_ino == rhs.st_ino && lhs.st_mode == rhs.st_mode &&
           ...
           lhs.st_flags == rhs.st_flags && lhs.st_mtimespec.tv_sec == rhs.st_mtimespec.tv_sec &&
           lhs.st_mtimespec.tv_nsec == rhs.st_mtimespec.tv_nsec;
}

// Scan's the given path, invoking the callback if the current scan state
// results in a mismatch.
static PathMap scanAndNotify(Path scanPath, const PathMap& oldState,
...
class FileSystemWatcherKQueue : public FileSystemWatcher {
  public:
    FileSystemWatcherKQueue(Path path, FileSystemWatcherCallback onChangeCallback)
        : FileSystemWatcher(onChangeCallback), mPath(path), mDirectoryView(scanDir(path)) {}
  ...
  private:
    bool watchForChanges() {
        ...
        if (event_fd <= 0) {
            LOG(ERROR) << "The file " << mPath << " could not be opened for monitoring. Error was "
                       << strerror(errno);
            close(mPipe[0]);
            close(mPipe[1]);
            mPipe[1] = -1;
        ...
        vnode_events = NOTE_DELETE | NOTE_WRITE | NOTE_EXTEND | NOTE_ATTRIB | NOTE_LINK |
                       NOTE_RENAME | NOTE_REVOKE;
        EV_SET(events_to_monitor + 1, event_fd, EVFILT_VNODE, EV_ADD | EV_ENABLE | EV_ONESHOT,
        ...
        while (mRunning) {
            int event_count =
                    kevent(kq, events_to_monitor, NUM_EVENT_FDS, event_data, num_files, 0);
            DD("Got %d events\n", event_count);
            if (mRunning) {
                ...
                if (event_count) {
                    DD("fd: %d, Event %" PRIdPTR
                       ...
                       event_fd, event_data[0].ident, event_data[0].filter, event_data[0].flags,
    ...
    void handleChange(int fd, struct kevent* event_data) {
        ...
        if (flags & NOTE_DELETE) {
            ...
            mDirectoryView = std::move(scanAndNotify(mPath, mDirectoryView, mChangeCallback));
        }
    }
...
class FileSystemWatcherFS : public FileSystemWatcher {
  ...
  private:
    static void watcherCb(ConstFSEventStreamRef, void* clientCallBackInfo, size_t numEvents,
        ...
        watcher->mDirectoryView = std::move(
                scanAndNotify(watcher->mPath, watcher->mDirectoryView, watcher->mChangeCallback));
    }
    ...
    bool watchForChanges() {
        ...
        auto stream = FSEventStreamCreate(
                nullptr, &FileSystemWatcherFS::watcherCb, &streamCtx, pathsToWatch,
