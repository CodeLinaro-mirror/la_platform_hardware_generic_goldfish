...
static PathMap
scanAndNotify(Path scanPath, const PathMap &oldState,
...
private:
  bool watchForChanges() {
    ...
    mStarted.signal();
    while (mRunning) {
      int event_count = kevent(kq, events_to_monitor, NUM_EVENT_FDS, event_data,
                               num_files, 0);
      DD("Got %d events\n", event_count);
      if (mRunning) {
  ...
  void handleChange(int fd, struct kevent *event_data) {
    ...
    if (flags & NOTE_DELETE) {
      ...
      mDirectoryView =
          std::move(scanAndNotify(mPath, mDirectoryView, mChangeCallback));
    }
  }
...
private:
  static void watcherCb(ConstFSEventStreamRef, void *clientCallBackInfo,
    ...
    watcher->mDirectoryView = std::move(scanAndNotify(
        watcher->mPath, watcher->mDirectoryView, watcher->mChangeCallback));
  }
