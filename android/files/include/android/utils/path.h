...
/* a wrapper for open(filename, oflag, pmode) to support unicode paths on
 * Windows */
 ...
 */
extern APosixStatus path_copy_file_safe(const char* dest, const char* source);
 ...
 * freed by the caller
 */
extern char* path_join(const char* part1, const char* part2);
...
/* copy folder, but skip file if it is in the skipset (type
   ...
   keeps copy
   */
extern APosixStatus path_copy_dir_ex(const char* dst, const char* src, void* pSkipSet);
