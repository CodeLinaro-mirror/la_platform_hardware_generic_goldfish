...
bool async(const ThreadFunctor& func, ThreadFlags flags) {
    auto thread = std::unique_ptr<SelfDeletingThread>(new SelfDeletingThread(func, flags));
    if (thread->start()) {
        thread.release();
        return true;
    }
...
class AsyncThreadWithLooper::Impl {
  public:
    Impl() : mLooper(Looper::create()), mThread([this] { threadFunc(); }) { mThread.start(); }
...
AsyncThreadWithLooper::AsyncThreadWithLooper() : mImpl(new AsyncThreadWithLooper::Impl) {}
