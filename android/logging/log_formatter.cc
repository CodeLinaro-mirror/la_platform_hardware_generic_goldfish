...
std::string SimpleLogFormatter::format(const LogParams& params, const std::string& line) {
    return absl::StrFormat("%s | %s", translate_sev(params.severity), line);
};

std::string SimpleLogWithTimeFormatter::format(const LogParams& params, const std::string& line) {
    struct timeval tv;
    gettimeofday(&tv, nullptr);
    time_t now = tv.tv_sec;
    struct tm* time = localtime(&now);
    return absl::StrFormat("%02d:%02d:%02d.%06ld %s | %s", time->tm_hour, time->tm_min,
                           time->tm_sec, tv.tv_usec, translate_sev(params.severity), line);
};
...
std::string GoogleLogFormatter::format(const LogParams& params, const std::string& line) {
    ...
    return absl::StrFormat("%c%02d%02d %02d:%02d:%02d.%06" PRId64 " %7s %s:%d] %s",
                           translate_sev(params.severity)[0], ts_parts.tm_mon + 1, ts_parts.tm_mday,
                           ts_parts.tm_hour, ts_parts.tm_min, ts_parts.tm_sec, microseconds,
                           getCachedThreadID(), filename, params.lineno, line);
}

std::string VerboseLogFormatter::format(const LogParams& params, const std::string& line) {
    struct timeval tv;
    gettimeofday(&tv, nullptr);
    time_t now = tv.tv_sec;
    struct tm* time = localtime(&now);
    ...
    auto logline = absl::StrFormat("%02d:%02d:%02d.%06d %s %s %-34s | %s", time->tm_hour,
                                   time->tm_min, time->tm_sec, static_cast<uint32_t>(tv.tv_usec),
                                   thread, translate_sev(params.severity), readable_location, line);
    return logline;
};
