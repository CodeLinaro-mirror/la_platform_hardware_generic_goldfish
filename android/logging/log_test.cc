...
class LogTest : public ::testing::Test, android::base::testing::LogOutput {
  public:
    ...
    ~LogTest() override { ::android::base::testing::LogOutput::setNewOutput(mSavedOutput); }
    ...
    // LogOutput override
    void logMessage(const LogParams& params, const char* message, size_t messageLen) override {
        mParams = params;
...
class PLogTest : public LogTest {
  public:
    ...
    void setExpectedErrno(LogSeverity severity, int line, int errnoCode, const char* suffix) {
        mExpectedParams.file = __FILE__;
        mExpectedParams.lineno = line;
        mExpectedParams.severity = severity;
        snprintf(mExpected, sizeof(mExpected), "%sError message: %s", suffix, strerror(errnoCode));
    }
...
#define EXPECTED_STRING_PREFIX(prefix, line) prefix ":" __FILE__ ":" STRINGIFY(line) ": "
