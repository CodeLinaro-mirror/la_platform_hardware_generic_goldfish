...
void __emu_log_print_str(LogSeverity prio, const char* file, int line, const std::string& msg) {
    write_log_line(prio, file, line, msg);
}

LOGGING_API extern "C" void __emu_log_print(LogSeverity prio, const char* file, int line,
                                            const char* fmt, ...) {
...
void logMessage(const LogParams& params, const char* message, size_t messageLen) {
    if (gLogOutput) {
