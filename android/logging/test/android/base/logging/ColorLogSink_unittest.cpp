  ...
  for (int i = static_cast<int>(absl::LogSeverity::kInfo);
    ...
    if (GetParam() == Console::TTY) {
      ...
      EXPECT_FALSE(colorString.empty())
          << "Color string is empty for severity "
          << absl::LogSeverityName(severity) << " when color (TTY) is enabled.";
    } else {
      // Color should be absent
      EXPECT_TRUE(colorString.empty())
          << "Color string is not empty for severity "
...
TEST_P(LogSinkTest, LogsInfo) {
  ...
  EXPECT_THAT(mStr.str(),
              ::testing::HasSubstr(mColorSink.Color(absl::LogSeverity::kInfo)));
}

TEST_P(LogSinkTest, LogsWarning) {
  LOG(WARNING) << "Hello world!";
  EXPECT_THAT(mStr.str(), ::testing::HasSubstr("Hello world!"));
  EXPECT_THAT(mStr.str(), ::testing::HasSubstr(
                              mColorSink.Color(absl::LogSeverity::kWarning)));
}
...
TEST_P(LogSinkTest, LogsError) {
  ...
  EXPECT_THAT(mStr.str(), ::testing::HasSubstr("Hello world!"));
  EXPECT_THAT(mStr.str(), ::testing::HasSubstr(
                              mColorSink.Color(absl::LogSeverity::kError)));
}
