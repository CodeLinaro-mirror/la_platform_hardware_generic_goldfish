...
TEST(LogFormatter, SimpleFormatterMatchesRegex) {
    ...
    const std::regex simple_log("^(VERBOSE|DEBUG|INFO|WARNING|ERROR|FATAL|UNKWOWN)\\s+\\| (.*)");
    std::smatch m;
    for (int sev = EMULATOR_LOG_DEBUG; sev < EMULATOR_LOG_NUM_SEVERITIES; sev++) {
        ...
        ASSERT_TRUE(std::regex_match(logline, m, simple_log));
...
TEST(LogFormatter, VerboseFormatterMatchesRegex) {
    ...
    for (int sev = EMULATOR_LOG_DEBUG; sev < EMULATOR_LOG_NUM_SEVERITIES; sev++) {
        ...
        LogParams lg = {"filename.c", 123, severity};
        std::string logline = formatter.format(lg, "Hello World");
        ASSERT_TRUE(std::regex_match(logline, m, verbose_log)) << "Logine = " << logline;
...
TEST(LogFormatter, GoogleFormatterMatchesRegex) {
    ...
    for (int sev = EMULATOR_LOG_DEBUG; sev < EMULATOR_LOG_NUM_SEVERITIES; sev++) {
        ...
        std::string logline = formatter.format(lg, "Hello World");
        ASSERT_TRUE(std::regex_match(logline, m, verbose_log)) << "Logine =" << logline;
        ASSERT_EQ(translate_sev(severity)[0], m[1]) << "Unexpected severity from:" << logline;
        ASSERT_EQ("filename.c:123", m[5]) << "Unexpected filename from:" << logline;
...
TEST(LogFormatter, DuplicateTripleLogsALine) {
    NoDuplicateLinesFormatter ndlf(std::make_shared<SimpleLogFormatter>());
    LogParams lg = {"filename.c", 123, EMULATOR_LOG_INFO};
...
TEST(LogFormatter, DuplicateTripleWithDiffLocationLogsALine) {
    NoDuplicateLinesFormatter ndlf(std::make_shared<SimpleLogFormatter>());
    LogParams lg = {"filename.c", 123, EMULATOR_LOG_INFO};
