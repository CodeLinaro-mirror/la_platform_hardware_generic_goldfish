...
TEST(IpAddressTest, AsV4ReturnsCorrectBinary) {
    const char* kIpV4Str = "192.168.1.100";
    ASSERT_OK_AND_ASSIGN(auto ip, IpAddress::create(kIpV4Str));
    ...
    ASSERT_EQ(1, inet_pton(AF_INET, kIpV4Str, &expected_addr))
            << "Test setup failed: inet_pton could not parse " << kIpV4Str;

    EXPECT_EQ(0, memcmp(v4_addr, &expected_addr, sizeof(expected_addr)));
}

TEST(IpAddressTest, AsV6ReturnsCorrectBinary) {
    const char* kIpV6Str = "2001:db8:85a3::8a2e:370:7334";
    ASSERT_OK_AND_ASSIGN(auto ip, IpAddress::create(kIpV6Str));
    ...
    ASSERT_EQ(1, inet_pton(AF_INET6, kIpV6Str, &expected_addr))
            << "Test setup failed: inet_pton could not parse " << kIpV6Str;

    EXPECT_EQ(0, memcmp(v6_addr, &expected_addr, sizeof(expected_addr)));
}

TEST(IpAddressTest, AsV6ReturnsCorrectBinaryLoopback) {
    const char* kIpV6Str = "::1";
    ASSERT_OK_AND_ASSIGN(auto ip, IpAddress::create(kIpV6Str));
