...
TEST(EndpointTest, CreateAndAccess) {
    ...
    EXPECT_THAT(endpoint.address().toString(), Eq("127.0.0.1"));
    EXPECT_THAT(endpoint.address().family(), Eq(IpAddress::Family::kIpv4));
    EXPECT_THAT(endpoint.port(), Eq(8080));
}

TEST(EndpointTest, ToStringIPv4) {
    ASSERT_OK_AND_ASSIGN(auto endpoint, Endpoint::create("192.168.1.1", 1234));
    EXPECT_THAT(endpoint.toString(), Eq("192.168.1.1:1234"));
}

TEST(EndpointTest, ToStringIPv6) {
    ASSERT_OK_AND_ASSIGN(auto endpoint, Endpoint::create("::2", 5678));
    EXPECT_THAT(endpoint.toString(), Eq("[::2]:5678"));
}
...
TEST(EndpointTest, ToSockaddrIPv4) {
    ...
    ASSERT_THAT(addr.ss_family, Eq(AF_INET));
    auto* sin = reinterpret_cast<sockaddr_in*>(&addr);
    EXPECT_THAT(ntohs(sin->sin_port), Eq(8080));

    char ip_str[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &sin->sin_addr, ip_str, sizeof(ip_str));
    EXPECT_THAT(ip_str, StrEq("192.168.1.2"));
}
...
TEST(EndpointTest, ToSockaddrIPv6) {
    ...
    ASSERT_THAT(addr.ss_family, Eq(AF_INET6));
    auto* sin6 = reinterpret_cast<sockaddr_in6*>(&addr);
    EXPECT_THAT(ntohs(sin6->sin6_port), Eq(12345));

    char ip_str[INET6_ADDRSTRLEN];
    inet_ntop(AF_INET6, &sin6->sin6_addr, ip_str, sizeof(ip_str));
    EXPECT_THAT(ip_str, StrEq("2001:db8::1"));
}
