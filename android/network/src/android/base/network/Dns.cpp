...
public:
    virtual int resolveName(std::string_view dns_server_name,
        ...
        if (ret != 0) {
            LOG(ERROR) << "getaddrinfo('" << dns_server_name.data() << "') returned " << ret;
            switch (ret) {
            ...
            for (address = addresses; address != NULL;
                ...
                for (dns_server = address->FirstDnsServerAddress;
                    ...
                    if (is_site_local_dns_broadcast(&dns_v6_addr->sin6_addr) ||
                        ...
                        LOG(INFO) << "Ignore IPv6 address: " << address_str;
                        continue;
                    }
                ...
                if (ip.valid()) {
                    ...
                    if (ip.isIpv6()) {
                        ...
                        if (!inet_pton(AF_INET6, nameserver, &sin6_addr)) {
                            LOG(INFO) << "Failed to parse IPv6 DNS address " << nameserver;
                        } else {
                    ...
                    if (!skip) {
                        ...
                        LOG(INFO) << "Ignore IPv6 link local address: " << ip.toString();
                    }
                }
            }
        }
