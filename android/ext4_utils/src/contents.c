...
static struct ext4_dir_entry_2* add_dentry(u8* data, u32* offset, struct ext4_dir_entry_2* prev,
                                           u32 inode, const char* name, u8 file_type) {
    ...
    if (start_block != end_block) {
        ...
        if (!prev) critical_error("no prev");
...
/* Creates a directory structure for an array of directory entries, dentries,
   ...
   new directory */
u32 make_directory(u32 dir_inode_num, u32 entries, struct dentry* dentries, u32 dirs) {
    ...
    for (i = 0; i < entries; i++) {
        dentry = add_dentry(data, &offset, dentry, 0, dentries[i].filename, dentries[i].file_type);
...
u32 make_file(const char* filename, u64 len) {
    ...
    if (len > 0) inode_allocate_file_extents(inode, len, filename);
 ...
 *    XATTR_CAPS_SUFFIX ("capability") because "selinux" is shorter than
 * "capability" 3) If the name_index and name_length are equal, then memcmp() is
 * used to determine which name comes first. For example, "selinux" would come
 * before "yelinux".
 *
...
static void xattr_assert_sane(struct ext4_xattr_entry* entry) {
    for (; !IS_LAST_ENTRY(entry); entry = EXT4_XATTR_NEXT(entry)) {
        ...
        if (cmp == 0) cmp = next->e_name_len - entry->e_name_len;
        if (cmp == 0) cmp = memcmp(next->e_name, entry->e_name, next->e_name_len);
        if (cmp < 0) {
...
static void ext4_xattr_hash_entry(struct ext4_xattr_header* header,
    ...
    if (entry->e_value_block == 0 && entry->e_value_size != 0) {
        u32* value = (u32*)((char*)header + le16_to_cpu(entry->e_value_offs));
        for (n = (le32_to_cpu(entry->e_value_size) + EXT4_XATTR_ROUND) >> EXT4_XATTR_PAD_BITS; n;
             n--) {
...
static struct ext4_xattr_entry* xattr_addto_range(void* block_start, void* block_end,
                                                  struct ext4_xattr_entry* first, int name_index,
    ...
    if (needed_size > available_size) return NULL;
...
static int xattr_addto_inode(struct ext4_inode* inode, int name_index, const char* name,
...
static int xattr_addto_block(struct ext4_inode* inode, int name_index, const char* name,
                             const void* value, size_t value_len) {
...
static int xattr_add(u32 inode_num, int name_index, const char* name, const void* value,
                     size_t value_len) {
    if (!value) return 0;
...
int inode_set_selinux(u32 inode_num, const char* secon) {
    ...
    return xattr_add(inode_num, EXT4_XATTR_INDEX_SECURITY, XATTR_SELINUX_SUFFIX, secon,
                     strlen(secon) + 1);
}

int inode_set_capabilities(u32 inode_num, uint64_t capabilities) {
    if (capabilities == 0) return 0;
    ...
    return xattr_add(inode_num, EXT4_XATTR_INDEX_SECURITY, XATTR_CAPS_SUFFIX, &cap_data,
                     sizeof(cap_data));
}
