...
void ext4_create_fs_aux_info() {
    ...
    aux_info.bg_desc_blocks =
            DIV_ROUND_UP(aux_info.groups * sizeof(struct ext2_group_desc), info.block_size);
...
void ext4_fill_in_sb() {
    ...
    if (info.feat_compat & EXT4_FEATURE_COMPAT_HAS_JOURNAL) sb->s_journal_inum = EXT4_JOURNAL_INO;
    sb->s_journal_dev = 0;
    sb->s_last_orphan = 0;
    ...
    sb->s_min_extra_isize = sizeof(struct ext4_inode) - EXT4_GOOD_OLD_INODE_SIZE;
    sb->s_want_extra_isize = sizeof(struct ext4_inode) - EXT4_GOOD_OLD_INODE_SIZE;
    sb->s_flags = 2;
    sb->s_raid_stride = 0;
    sb->s_mmp_interval = 0;
    sb->s_mmp_block = 0;
    ...
    for (i = 0; i < aux_info.groups; i++) {
        u64 group_start_block = aux_info.first_data_block + i * info.blocks_per_group;
        u32 header_size = 0;
        if (ext4_bg_has_super_block(i)) {
            if (i != 0) {
                ...
                sparse_file_add_data(ext4_sparse_file, aux_info.backup_sb[i], info.block_size,
                                     group_start_block);
            }
            sparse_file_add_data(ext4_sparse_file, aux_info.bg_desc,
                                 aux_info.bg_desc_blocks * info.block_size, group_start_block + 1);
...
void ext4_create_journal_inode() {
    ...
    u8* journal_data = inode_allocate_data_extents(inode, info.journal_blocks * info.block_size,
