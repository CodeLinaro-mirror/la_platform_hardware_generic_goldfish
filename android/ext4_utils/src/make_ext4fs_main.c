...
int main(int argc, char** argv) {
    ...
    if (wipe && sparse) {
        fprintf(stderr, "Cannot specify both wipe and sparse\n");
        usage(argv[0]);
        exit(EXIT_FAILURE);
    }

    if (wipe && gzip) {
        fprintf(stderr, "Cannot specify both wipe and gzip\n");
        usage(argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    exitcode = make_ext4fs_internal(fd, directory, mountpoint, fs_config_func, gzip, sparse, crc,
                                    wipe, sehnd, verbose);
    close(fd);
    if (exitcode && strcmp(filename, "-")) unlink(filename);
    return exitcode;
}
