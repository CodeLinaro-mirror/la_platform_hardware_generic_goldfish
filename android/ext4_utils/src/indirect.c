...
static u8* create_backing(struct block_allocation* alloc, unsigned long backing_len) {
    ...
    if (!data) critical_error_errno("calloc");
...
static void fill_dindirect_block(u32* dind_block, int len, struct block_allocation* alloc) {
    ...
    for (i = 0; len > 0; i++) {
        ...
        sparse_file_add_data(ext4_sparse_file, ind_block_data, info.block_size, ind_block);
        int ind_block_len = min((int)aux_info.blocks_per_ind, len);
...
static void fill_tindirect_block(u32* tind_block, int len, struct block_allocation* alloc) {
    ...
    for (i = 0; len > 0; i++) {
        ...
        sparse_file_add_data(ext4_sparse_file, dind_block_data, info.block_size, dind_block);
        int dind_block_len = min((int)aux_info.blocks_per_dind, len);
...
static int inode_attach_direct_blocks(struct ext4_inode* inode, struct block_allocation* alloc,
                                      u32* block_len) {
...
static int inode_attach_indirect_blocks(struct ext4_inode* inode, struct block_allocation* alloc,
                                        u32* block_len) {
...
static int inode_attach_dindirect_blocks(struct ext4_inode* inode, struct block_allocation* alloc,
                                         u32* block_len) {
...
static int inode_attach_tindirect_blocks(struct ext4_inode* inode, struct block_allocation* alloc,
                                         u32* block_len) {
    ...
    sparse_file_add_data(ext4_sparse_file, tind_block_data, info.block_size, tind_block);
...
static void reserve_all_indirect_blocks(struct block_allocation* alloc, u32 len) {
    if (len <= EXT4_NDIR_BLOCKS) return;
...
static u32 indirect_blocks_needed(u32 len) {
    ...
    if (len <= EXT4_NDIR_BLOCKS) return ind;
    ...
    if (len <= aux_info.blocks_per_ind) return ind;

    len -= aux_info.blocks_per_ind;

    ind += DIV_ROUND_UP(len, aux_info.blocks_per_dind);

    if (len <= aux_info.blocks_per_dind) return ind;

    len -= aux_info.blocks_per_dind;

    ind += DIV_ROUND_UP(len, aux_info.blocks_per_tind);

    if (len <= aux_info.blocks_per_tind) return ind;

    critical_error("request too large");
    return 0;
}

static int do_inode_attach_indirect(struct ext4_inode* inode, struct block_allocation* alloc,
                                    u32 block_len) {
    u32 count = block_len;
...
static struct block_allocation* do_inode_allocate_indirect(u32 block_len) {
...
void inode_attach_resize(struct ext4_inode* inode, struct block_allocation* alloc) {
    ...
    if (block_len % info.bg_desc_reserve_blocks)
        critical_error("reserved blocks not a multiple of %d", info.bg_desc_reserve_blocks);

    append_oob_allocation(alloc, 1);
    u32 dind_block = get_oob_block(alloc, 0);

    u32* dind_block_data = calloc(info.block_size, 1);
    if (!dind_block_data) critical_error_errno("calloc");
    sparse_file_add_data(ext4_sparse_file, dind_block_data, info.block_size, dind_block);

    u32* ind_block_data = calloc(info.block_size, info.bg_desc_reserve_blocks);
    if (!ind_block_data) critical_error_errno("calloc");
    sparse_file_add_data(ext4_sparse_file, ind_block_data,
                         info.block_size * info.bg_desc_reserve_blocks, get_block(alloc, 0));
    ...
    u32 last_block = EXT4_NDIR_BLOCKS + aux_info.blocks_per_ind +
                     aux_info.blocks_per_ind * (info.bg_desc_reserve_blocks - 1) + superblocks - 2;
...
u8* inode_allocate_data_indirect(struct ext4_inode* inode, unsigned long len,
    ...
    if (backing_len) {
        ...
        if (!data) error("failed to create backing for %lu bytes", backing_len);
    }
