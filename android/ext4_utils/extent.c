...
static u8* extent_create_backing(struct block_allocation* alloc, u64 backing_len) {
...
/* Queues each chunk of a file to be written to contiguous data block
   regions */
static void extent_create_backing_file(struct block_allocation* alloc, u64 backing_len,
                                       const char* filename) {
    off64_t offset = 0;
    for (; alloc != NULL && backing_len > 0; get_next_region(alloc)) {
        u32 region_block;
        ...
        sparse_file_add_file(ext4_sparse_file, filename, offset, len, region_block);
        offset += len;
        backing_len -= len;
    }
}

static struct block_allocation* do_inode_allocate_extents(struct ext4_inode* inode, u64 len) {
    ...
    if (!extent_block) {
        ...
        struct ext4_extent_idx* idx = (struct ext4_extent_idx*)&inode->i_block[3];
        idx->ei_block = 0;
        ...
        sparse_file_add_data(ext4_sparse_file, data, info.block_size, extent_block);
        ...
        hdr->eh_entries = allocation_len;
        hdr->eh_max =
                (info.block_size - sizeof(struct ext4_extent_header)) / sizeof(struct ext4_extent);
        hdr->eh_generation = 0;
        hdr->eh_depth = 0;
...
/* Allocates enough blocks to hold len bytes, with backing_len bytes in a data
   ...
   buffer. */
u8* inode_allocate_data_extents(struct ext4_inode* inode, u64 len, u64 backing_len) {
    ...
    if (backing_len) {
        ...
        if (!data) error("failed to create backing for %" PRIu64 " bytes", backing_len);
    }
...
void inode_allocate_file_extents(struct ext4_inode* inode, u64 len, const char* filename) {
    struct block_allocation* alloc;
