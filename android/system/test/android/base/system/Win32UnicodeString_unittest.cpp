...
TEST(Win32UnicodeString, convertToUtf8) {
  ...
  for (size_t n = 0; n < kDataSize; ++n) {
    ...
    char out[256];
    int len =
        Win32UnicodeString::convertToUtf8(out, sizeof(out), kData[n].utf16);
...
TEST(Win32UnicodeString, convertFromUtf8) {
  ...
  for (size_t n = 0; n < kDataSize; ++n) {
    ...
    len = Win32UnicodeString::convertFromUtf8(
        out, ARRAY_SIZE(out), kData[n].utf8, strlen(kData[n].utf8));
