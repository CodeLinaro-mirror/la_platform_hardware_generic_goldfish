...
*/
#ifdef __linux__ /* Recent versions of glibc only define EAI_NODATA, which is \
                    an extension to the POSIX standard, if _GNU_SOURCE is     \
                    defined. */
...
const char* _errno_str(void) {
    ...
    if (result == NULL) {
        result = tempstr_format("Unknown socket error (Winsock=0x%08x) errno=%d: %s", winsock_error,
...
inline static void fd_set_cloexec(int fd) {
#ifndef _WIN32
...
static int socket_family_to_bsd(SocketFamily family) {
    switch (family) {
        case SOCKET_INET:
            return AF_INET;
        case SOCKET_IN6:
...
static int socket_type_to_bsd(SocketType type) {
    switch (type) {
        case SOCKET_DGRAM:
            return SOCK_DGRAM;
        case SOCKET_STREAM:
...
static SocketType socket_type_from_bsd(int type) {
    switch (type) {
        case SOCK_DGRAM:
            return SOCKET_DGRAM;
        case SOCK_STREAM:
        ...
        default:
            return (SocketType)SOCKET_UNSPEC;
    }
}
...
static char* format_char(char* buf, char* end, int c) {
    if (buf < end) {
        if (buf + 1 == end) {
            *buf++ = 0;
        } else {
...
static char* format_str(char* buf, char* end, const char* str) {
    int len = strlen(str);
    int avail = end - buf;
...
static char* format_unsigned(char* buf, char* end, unsigned val) {
    char temp[16];
    int nn;
    ...
    while (nn > 0) buf = format_char(buf, end, temp[--nn]);

    return buf;
}

static char* format_ip4(char* buf, char* end, uint32_t ip) {
...
static char* format_ip6(char* buf, char* end, const uint8_t* ip6) {
    struct sockaddr_in6 sa[1];
    memset(sa, 0, sizeof(sa));
    sa->sin6_family = AF_INET6;
    memcpy(sa->sin6_addr.s6_addr, ip6, 16);
    if (getnameinfo((const struct sockaddr*)sa, sizeof(sa), buf, end - buf, NULL, 0,
                    NI_NUMERICHOST))
        return buf;
...
const char* sock_address_host_string(const SockAddress* a, char* buf0, size_t len) {
...
const char* sock_address_to_string(const SockAddress* a, char* buf0, size_t len) {
    char *buf = buf0, *end = buf + len;
...
int sock_address_equal(const SockAddress* a, const SockAddress* b) {
    ...
    switch (a->family) {
        case SOCKET_INET:
            return (a->u.inet.address == b->u.inet.address && a->u.inet.port == b->u.inet.port);
...
int sock_address_get_ip(const SockAddress* a) {
    if (a->family == SOCKET_INET) return a->u.inet.address;
...
static int sock_address_to_bsd(const SockAddress* a, sockaddr_storage* paddress, socklen_t* psize) {
    switch (a->family) {
        case SOCKET_INET: {
        ...
        case SOCKET_UNIX: {
            ...
            if (slen >= UNIX_PATH_MAX) return -1;
...
static int sock_address_from_bsd(SockAddress* a, const void* from, size_t fromlen) {
    switch (((struct sockaddr*)from)->sa_family) {
        case AF_INET: {
            const struct sockaddr_in* src = from;

            if (fromlen < sizeof(*src)) return set_errno(EINVAL);
        ...
        case AF_LOCAL: {
            ...
            if (fromlen < sun_path_offset + 1) return set_errno(EINVAL);
            ...
            if (!abstract_path) {
                ...
                if (end == NULL) return set_errno(EINVAL);
 ...
 */
    ...
    /* Parse the returned list of addresses. */
    {
        ...
        if (preferIn6) {
            r = res_ipv6;
            if (r == NULL) r = res_ipv4;
        } else {
            r = res_ipv4;
            if (r == NULL) r = res_ipv6;
        }
...
SockAddress** sock_address_list_create(const char* hostname, const char* port, unsigned flags) {
    ...
    if (flags & SOCKET_LIST_DGRAM) ai.ai_socktype = SOCK_DGRAM;
    ...
    for (count = 0, e = res; e != NULL; e = e->ai_next) count += 1;
...
SockAddress** sock_address_list_create2(const char* host_and_port, unsigned flags) {
    char host_name[512];
...
int sock_address_get_numeric_info(SockAddress* a, char* host, size_t hostlen, char* serv,
                                  size_t servlen) {
    ...
    ret = getnameinfo(saddr, slen, host, hostlen, serv, servlen, NI_NUMERICHOST | NI_NUMERICSERV);
...
int socket_sendto(int fd, const void* buf, int buflen, const SockAddress* to) {
    ...
    if (sock_address_to_bsd(to, &sa, &salen) < 0) return -1;
...
int socket_recvfrom(int fd, void* buf, int len, SockAddress* from) {
    sockaddr_storage sa;
    ...
    if (sock_address_from_bsd(from, &sa, salen) < 0) return -1;

    return ret;
}

int socket_connect_no_sigalrm(int socket, const void* address, uint32_t address_len) {
    int ret;
    BEGIN_NOSIGALRM
...
int socket_connect(int fd, const SockAddress* address) {
    ...
    if (sock_address_to_bsd(address, &addr, &addrlen) < 0) return -1;
...
int socket_connect_posix(int fd, const void* address, uint32_t address_len) {
    SOCKET_CALL(
            socket_connect_no_sigalrm(fd, (const struct sockaddr*)address, (socklen_t)address_len));
}
...
static int socket_getoption(int fd, int domain, int option, int default_) {
    int ret;
    while (1) {
#ifdef _WIN32
        DWORD opt = (DWORD)-1;
        ...
        ret = HANDLE_EINTR(getsockopt(fd, domain, option, (char*)&opt, &optlen));
        if (ret == 0)
            return (int)opt;
        else
            return default_;
...
static int socket_setoption(int fd, int domain, int option, int _flag) {
#ifdef _WIN32
...
int socket_set_xreuseaddr(int fd) {
     ...
     * semantics. instead of SO_EXCLUSIVEADDR to ensure that explicitly prevent
     * this.
     */
...
static int socket_bind_server(int s, const SockAddress* to, SocketType type) {
    ...
    if (type == SOCKET_STREAM) {
        if (socket_listen(s, 4) < 0) {
            D("could not listen server socket %s: %s", sock_address_to_string(to, tmp, sizeof(tmp)),
              errno_str);
            goto FAIL;
        }
    }
...
static int socket_connect_client(int s, const SockAddress* to) {
...
static int socket_in_server(int address, int port, SocketType type) {
    SockAddress addr;
    int s;
...
static int socket_in_client(SockAddress* to, SocketType type) {
    int s;
...
int socket_network_client(const char* host, int port, SocketType type) {
    ...
    if (sock_address_init_resolve(&addr, host, port, 0) < 0) return -1;
...
int socket_accept_any(int server_fd) {
    ...
    if (fd < 0) {
        D("could not accept client connection from fd %d: %s", server_fd, errno_str);
        return -1;
    }
...
int socket_mcast_inet_add_membership(int s, uint32_t ip) {
    ...
    if (setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, (const char*)&imr, sizeof(struct ip_mreq)) <
        0) {
        return fix_errno();
    }
    return 0;
...
int socket_mcast_inet_drop_membership(int s, uint32_t ip) {
    ...
    if (setsockopt(s, IPPROTO_IP, IP_DROP_MEMBERSHIP, (const char*)&imr, sizeof(struct ip_mreq)) <
        0) {
        return fix_errno();
    }
    return 0;
...
int qemu_getsockopt(int sock, int level, int optname, void* optval, size_t* optlen) {
...
int qemu_setsockopt(int sock, int level, int optname, const void* optval, size_t optlen) {
    return setsockopt(sock, level, optname, (const char*)optval, (socklen_t)optlen);
}
