...
extern void stralloc_ready(stralloc_t* s, unsigned int len) {
    unsigned old_max = s->a;
    unsigned new_max = old_max;

    while (new_max < len) {
        unsigned new_max2 = new_max + (new_max >> 1) + 16;
        if (new_max2 < new_max) new_max2 = UINT_MAX;
        new_max = new_max2;
    }
...
extern void stralloc_readyplus(stralloc_t* s, unsigned int len) {
...
void stralloc_lstrip(stralloc_t* s) {
    int count;

    for (count = 0; count < s->n; count++) {
        if (s->s[count] != ' ' && s->s[count] != '\t') break;
    }
...
void stralloc_rstrip(stralloc_t* s) {
    int count = s->n;

    while (count > 0 && (s->s[count - 1] == ' ' || s->s[count - 1] == '\t')) count--;

    s->n = count;
}

void stralloc_strip(stralloc_t* s) {
    stralloc_rstrip(s);
    stralloc_lstrip(s);
}
