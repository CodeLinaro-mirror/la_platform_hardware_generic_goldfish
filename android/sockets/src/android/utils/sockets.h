...
typedef enum { SOCKET_DGRAM = 0, SOCKET_STREAM } SocketType;

/* supported socket families */
typedef enum { SOCKET_UNSPEC, SOCKET_INET, SOCKET_IN6, SOCKET_UNIX } SocketFamily;
...
/* Initialize a Unix socket address, this will copy the 'path' string into the
...
static __inline__ SocketFamily sock_address_get_family(const SockAddress* a) {
    return a->family;
}
...
/* set the port number of a given socket address, don't do anything for Unix
 * ones */
...
/* create a new socket intended for IPv4 communication. returns the socket
 * number, or -1 on failure.
 */
int socket_create_inet(SocketType type);

/* create a new socket intended for IPv6 communication. returns the socket
 * number, or -1 on failure.
 */
#if HAVE_IN6_SOCKETS
