...
class BaseSocketServer : public AsyncSocketServer {
  public:
    BaseSocketServer(Looper* looper, int port, ConnectCallback connectCallback, int socket4,
                     int socket6)
  ...
  private:
    ...
    static void onAccept(void* opaque, int fd, unsigned events) {
        ...
        if (!server->isListening()) {
            // If we listen on IPv4 and IPv6 both, then we could get onAccept from one
            // socket even we've already called stopListening on another socket if we
            // get connections from both sockets around same time. In such case, just
            // ignore onAccept. b/150887008 CHECK(server->getListenMode() ==
            // kIPv4AndIPv6) << "Hit onAccept after stopListening";
            return;
        }
...
std::unique_ptr<AsyncSocketServer> AsyncSocketServer::createTcpLoopbackServer(
        int port, ConnectCallback connectCallback, LoopbackMode mode, Looper* looper) {
    if (!looper) {
    ...
    if (success && (sock4.valid() || sock6.valid())) {
        result = new BaseSocketServer(looper, port, connectCallback, sock4.release(),
                                      sock6.release());
    }
