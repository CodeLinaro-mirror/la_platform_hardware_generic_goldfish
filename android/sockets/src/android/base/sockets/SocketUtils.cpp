...
void winsockCleanup() { WSACleanup(); }
...
void socketInitWinsock() {
  ...
  LONG status =
      InterlockedCompareExchange(&sWinsockInit, INITIALIZING, UNINITIALIZED);
  if (status == COMPLETED) {
  ...
  while (status == INITIALIZING) {
    ::Sleep(0);
    status =
        InterlockedCompareExchange(&sWinsockInit, INITIALIZING, INITIALIZING);
  }
}
...
union SockAddressStorage {
  ...
  static bool resolveHostNameToList(const char* hostName,
...
int socketSetOption(int socket, int domain, int option, int _flag) {
  ...
  errno = 0;
  int ret =
      ::setsockopt(socket, domain, option, (const char*)&flag, sizeof(flag));
...
#endif
  ssize_t ret = ::send(socket, reinterpret_cast<const char*>(buffer), bufferLen,
                       sendFlags);
  ON_SOCKET_ERROR_RETURN_M1(ret);
  return ret;
}
...
static int socketTcpLoopbackClientFor(int port, int domain) {
  ...
  if (connres < 0 &&
    ...
    if (ret != 0) {
      ...
      PLOG(ERROR) << ": failed eventsetup! res: " << ret << " err " << err
                  << " 0x" << std::hex << err;
      return -1;
    }
    ...
    if (ret == WAIT_FAILED) {
      ...
      PLOG(ERROR) << ": WaitForSingleObject failed. err " << err << " 0x"
                  << std::hex << err;
      return -1;
    }
...
int socketCreateTcp4() { return socketCreateTcpFor(AF_INET); }

int socketCreateTcp6() { return socketCreateTcpFor(AF_INET6); }
