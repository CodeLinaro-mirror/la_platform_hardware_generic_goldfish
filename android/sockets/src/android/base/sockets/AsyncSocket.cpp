...
void AsyncSocket::connectToPort() {
    int socket = 0;
    VLOG(1) << "Connecting to localhost:" << mPort;
    while (socket < 1 && mConnecting) {
        ...
        if (socket < 0) {
            VLOG(1) << "Failed to connect to localhost:" << mPort << ", sleeping 1s.";
            std::this_thread::sleep_for(std::chrono::seconds(1));
        }
    }
    if (socket < 1) {
        VLOG(1) << "giving up connecting to localhost:" << mPort;
        return;
    }
    ...
    VLOG(1) << "Connected to localhost:" << mPort;
