...
static void extractPartitionFormat(PartitionConfigState* state, const std::string& fstab,
    ...
    if (!state->backend->parsePartitionFormat(fstab, partitionPath, &partFormat)) {
        dprint("Could not extract format of %s partition!", partitionName);
        return;
    }
    dprint("Found format of %s partition: '%s'", partitionName, partFormat.c_str());
...
static auto addNandImage(PartitionConfigState* state, const char* part_name,
    ...
    // Verify partition type, or probe it if needed.
    {
        ...
        if (part_type == ANDROID_PARTITION_TYPE_UNKNOWN) {
            dprint("Probing %s image file for partition type: %s", part_name, image_file);
            ...
            if (image_type != part_type) {
                ...
                dprint(
                    ...
                    "%s (expected %s)",
                    part_name, androidPartitionType_toString(image_type),
                    androidPartitionType_toString(part_type));
    ...
    dprint("%s partition format: %s", part_name, androidPartitionType_toString(part_type));
    ...
    if (readonly) {
        ...
        if (need_temp_partition) {
            ...
            dprint("Mapping '%s' partition image to %s", part_name, part_file);
    ...
    if (need_make_empty) {
        dprint("Creating empty %s partition image at: %s", part_name, part_file);
        int ret =
            static_cast<int>(state->backend->makeEmptyPartition(part_type, part_size, part_file));
        if (ret < 0) {
...
auto android_partition_configuration_setup(const AndroidPartitionConfiguration* config,
...

    {
        ...
        if (state->backend->extractRamdiskFile(config->ramdisk_path, config->fstab_name, &fstab)) {
            dprint("Ramdisk image contains %s file", config->fstab_name);
            ...
            dprint("No %s file in ramdisk image", config->fstab_name);
        }
    }
    ...
    if ((config->wipe_data || current_data_size < config->data_partition.size) &&
        ...
        if (current_data_size < config->data_partition.size) {
            dprint("userdata partition is resized from %d M to %d M\n",
                   current_data_size.bytes() / (1024 * 1024), config->data_partition.size / (1024 * 1024));
        }
