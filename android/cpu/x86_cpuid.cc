...
uint32_t android_get_x86_cpuid_extended_function_max() {
    uint32_t function_max;
    android_get_x86_cpuid(0x80000000, 0, &function_max, nullptr, nullptr, nullptr);
    return function_max;
}
...
bool android_get_x86_cpuid_vendor_id_is_vmhost(const char* vendor_id) {
    ...
    for (int i = 0; i < VMHostCPUIDCount; i++) {
        ...
        if (strcmp(vendor_id, VMHostCPUID[i]) == 0) return true;
    }
    return false;
}
...
void android_get_x86_cpuid_vmhost_vendor_id(char* buf, size_t buf_len) {
    ...
    android_get_x86_cpuid(0x40000000, 0, nullptr, (uint32_t*)buf, (uint32_t*)(buf + 4),
                          (uint32_t*)(buf + 8));
    ...
    if (strcmp(buf, "Microsoft Hv") == 0) {
        android_get_x86_cpuid(0x40000100, 0, nullptr, (uint32_t*)_buf, (uint32_t*)(_buf + 4),
                              (uint32_t*)(_buf + 8));
        if (_buf[0]) memcpy(buf, _buf, 13);
    }
}
...
bool android_get_x86_cpuid_nx_support() {
    if (android_get_x86_cpuid_extended_function_max() < 0x80000001) return false;

    uint32_t cpuid_80000001_edx;
    android_get_x86_cpuid(0x80000001, 0, nullptr, nullptr, nullptr, &cpuid_80000001_edx);
...
bool android_get_x86_cpuid_is_64bit_capable() {
    ...
    uint32_t edx = 0;
    android_get_x86_cpuid(kBitnessSupportFunc, 0, nullptr, nullptr, nullptr, &edx);
...
bool android_get_x86_cpuid_hyperv_root() {
    ...
    android_get_x86_cpuid(0x40000003, 0, nullptr, &cpuid_function40000003_ebx, nullptr, nullptr);
