...
TEST(StorageCapacityTest, ParseValidStrings) {
  ...
  EXPECT_EQ(result_tb.value().bytes(),
               4 * 1024ULL * 1024ULL * 1024ULL * 1024ULL);
}
...
TEST(StorageCapacityTest, ParseInvalidStrings) {
  ...
  absl::StatusOr<StorageCapacity> result_invalid =
      StorageCapacity::parse("invalid");
...
TEST(StorageCapacityTest, StringRepresentation) {
  ...
  EXPECT_EQ(StorageCapacity(1, StorageCapacity::Unit::TiB).string(),
            "1.00 TiB");
}
