...
TEST(System, granularity) {
  ...
  LOG(INFO) << "Counted " << diff << " slices in " << (now - start)
            << " us, a slice is +/- " << resolution << " us.";
  EXPECT_GT(diff, 1000);
}
...
TEST(System, recursiveSize) {
  ...
  static const System::FileSize kFileSizes[] = {
      123,  55,    2345,   2222,   3333,    8329, 472,
  ...
  EXPECT_EQ(kFileSizes[0], System::get()->recursiveSize(
                               PathUtils::join(myDir->path(), kFiles[0])));
}
...
TEST(System, pathOperations) {
  ...
  // is readable.
  EXPECT_FALSE(
      HANDLE_EINTR(chmod(fooPath.c_str(), S_IRUSR | S_IWUSR | S_IXUSR)));
