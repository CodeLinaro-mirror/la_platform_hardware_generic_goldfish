...
TEST(bazel_info, can_get_data_file) {
    EXPECT_FALSE(Bazel::RunfilesPath("goldfish+/emulator/libs/system/testdata/info.txt").empty());
}

TEST(bazel_info, can_read_data_file) {
    std::string path = Bazel::RunfilesPath("goldfish+/emulator/libs/system/testdata/info.txt");
