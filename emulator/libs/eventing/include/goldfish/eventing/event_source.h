...
#include "goldfish/eventing/policies/dispatch_policies.h"
...
class EventListener {
  public:
     ...
     */
    virtual void EventArrived(typename EventParam<T>::type event) = 0;
};
 ...
 */
template <class T, class StoragePolicy, class DispatcherPolicy = NonBlockingDispatcher>
class EventSource {
  public:
  ...
  private:
    typename StoragePolicy::Container listeners_;
    std::mutex listener_lock_;
    DispatcherPolicy dispatcher_;

  public:
    template <typename... Args>
    explicit EventSource(Args&&... args) : dispatcher_(std::forward<Args>(args)...) {}

    /**
     * @brief Adds a listener to the source.
     * @param listener The listener to add.
     */
    void AddListener(const Ptr& listener) {
        const std::lock_guard<std::mutex> lock(listener_lock_);
        StoragePolicy::Add(listeners_, listener);
    }
     ...
     */
    void RemoveListener(const Ptr& listener) {
        const std::lock_guard<std::mutex> lock(listener_lock_);
        StoragePolicy::Remove(listeners_, listener);
    }
    ...
    void FireEvent(const T& event) {
        dispatcher_.template Dispatch<T, StoragePolicy>(event, listeners_, listener_lock_);
    }
    ...
    size_t Size() {
        const std::lock_guard<std::mutex> lock(listener_lock_);
        return StoragePolicy::Size(listeners_);
    }
    ...
    void Clear() {  // NOLINT
        const std::lock_guard<std::mutex> lock(listener_lock_);
        StoragePolicy::Clear(listeners_);
    }
};
...
class ConcurrentEventSource {
  public:
     ...
     * @param listener The listener to add.
     */
    void AddListener(Ptr listener) { SelfSyncStoragePolicy::Add(listeners_, listener); }

    /**
     * @brief Removes a listener from the source.
     * @param listener The listener to remove.
     */
    void RemoveListener(Ptr listener) { SelfSyncStoragePolicy::Remove(listeners_, listener); }
     ...
     */
    void FireEvent(const T& event) {
        const std::vector<Ptr> listeners_copy = SelfSyncStoragePolicy::Copy(listeners_);
     ...
     * @return The current number of registered listeners.
     */
    size_t Size() { return SelfSyncStoragePolicy::Size(listeners_); }  // NOLINT

    /**
     * @brief Removes all listeners from the source.
     */
    void Clear() { SelfSyncStoragePolicy::Clear(listeners_); }  // NOLINT

  private:
    typename SelfSyncStoragePolicy::Container listeners_;
};
