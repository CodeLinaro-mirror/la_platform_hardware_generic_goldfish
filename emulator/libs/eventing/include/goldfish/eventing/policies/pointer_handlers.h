...
struct PointerHandlers {
    ...
    static auto find(Iterator begin, Iterator end, const PtrType& val) {  // NOLINT
    ...
    static size_t count_live(const Container& c) {  // NOLINT
        if constexpr (HasSizeMethod<const Container&>::value) {
            return c.size();
        } else {
...
struct PointerHandlers<std::weak_ptr<T>> {
     ...
     */
    template <typename Iterator>
    static auto find(Iterator begin, Iterator end, const Ptr& val) {  // NOLINT
...
template <typename T>
struct EventParam {
    using type = std::conditional_t<std::is_pointer_v<T> || std::is_fundamental_v<T>, T, const T&>;
};
