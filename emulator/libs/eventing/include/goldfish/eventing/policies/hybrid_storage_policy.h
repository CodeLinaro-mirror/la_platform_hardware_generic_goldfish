...
#include "goldfish/eventing/policies/vector_storage_policy.h"
...
struct StaticArray {
    ...
    const T* begin() const { return data; }       // NOLINT
    const T* end() const { return data + size; }  // NOLINT
};
 ...
 *
 * When the number of listeners exceeds the static capacity (`Capacity`), the policy
 ...
 * @tparam Capacity The capacity of the on-stack buffer before promoting.
 ...
 * @code
 * #include "goldfish/eventing/policies/hybrid_storage_policy.h"
 * #include "goldfish/eventing/policies/unordered_set_storage_policy.h"
 *
 ...
 * @endcode
 */
template <class T, int Capacity = 16, class PointerType = EventListener<T>*,
          class PromotionPolicy = VectorStoragePolicy<T, PointerType>>
struct HybridStoragePolicy {
    using Ptr = PointerType;
    using StaticContainer = StaticArray<Ptr, Capacity>;
                    ...
                    if constexpr (std::is_same_v<StorageType, StaticContainer>) {
                        ...
                        if (PointerHandlers<Ptr>::find(storage.data, end, listener) == end) {
                            if (storage.size < Capacity) {
                                ...
                                for (size_t i = 0; std::cmp_less(i, Capacity); ++i) {
...
template <class T, int Capacity = 16, class PointerType = EventListener<T>*,
          class PromotionPolicy = VectorStoragePolicy<T, PointerType>>
using HybridSource = EventSource<T, HybridStoragePolicy<T, Capacity, PointerType, PromotionPolicy>>;
...
using BlockingHybridSource =
        BlockingEventSource<T, HybridStoragePolicy<T, Capacity, PointerType, PromotionPolicy>>;
}  // namespace android::base::eventing