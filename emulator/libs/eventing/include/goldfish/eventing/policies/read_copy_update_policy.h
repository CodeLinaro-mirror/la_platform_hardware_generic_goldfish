...
struct ReadCopyUpdatePolicy {
    ...
    static void Add(Container& storage, const Ptr& listener) {
        ...
        if (PointerHandlers<Ptr>::find(new_listeners.begin(), new_listeners.end(), listener) ==
            new_listeners.end()) {
...
template <class T>
using WeakPtrRcuSource =
        ConcurrentEventSource<T, ReadCopyUpdatePolicy<T, std::weak_ptr<EventListener<T>>>>;
