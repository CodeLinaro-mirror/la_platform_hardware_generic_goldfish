 ...
 * ======================================================================================================================
 *
 * This file contains benchmarks for various event source configurations. The results below provide
 * a clear guide on which policies and sources to use for different scenarios.
 *
 * Conclusion:
 * - For single-threaded, fire-heavy workloads: Vector-based policies (`VectorSource`,
 * `FastEventSource`) are the fastest.
 * - For write-heavy workloads: `UnorderedSetSource` is the clear winner due to its O(1) add/remove
 * complexity.
 * - For multi-threaded, high-contention workloads: `HighContentionEventSource` (using Abseil's
 * read-write lock) and `RcuSource` are the best choices as they scale well with multiple threads.
 * - For memory safety: `SafeEventSource` and `CallbackEventSource` (which uses `SafeEventSource`)
 * provide protection against dangling pointers at a significant performance cost.
 *
 * Detailed Breakdown:
 *
 * 1. FireEvent (Single-Threaded Read Performance):
 *    - WINNER: Vector-based policies (`VectorSource`, `FastEventSource`) are by far the fastest due
 * to excellent cache locality.
 *    - MIDDLE: `UnorderedSetSource` is slower due to pointer chasing. `RcuSource` has some
 * overhead.
 *    - SLOWEST: `WeakPtr` policies (`SafeEventSource`, `CallbackEventSource`) are significantly
 * slower due to the overhead of locking weak pointers.
 *
 * 2. AddListener (Write Performance):
 *    - WINNER: `UnorderedSetSource` provides the best performance, especially as the number of
 * listeners grows.
 *    - MIDDLE: `OrderedVectorSource` is a good all-rounder, faster than a plain vector for large N.
 *    - SLOWEST: `VectorSource` and `RcuSource` degrade significantly as N increases because they
 * need to scan or copy the entire list on each insertion. `WeakPtr` policies are also very slow.
 *
 * 3. Read Contention (Multi-threaded Read Performance):
 *    - WINNER: `RcuSource` and `HighContentionEventSource` (Abseil) scale the best, showing minimal
 * performance degradation as threads are added.
 *    - LOSER: All policies that use a single `std::mutex` (`Vector`, `UnorderedSet`, etc.) become
 * bottlenecks and perform poorly under high read contention.
 *
 * 4. Write Contention (Multi-threaded Write Performance):
 *    - WINNER: `HighContentionEventSource` (Abseil) is the clear winner, providing excellent
 * performance even with many threads trying to write simultaneously.
 *    - LOSER: All other policies, especially those based on vectors, suffer heavily from lock
 * contention.
 *
 * 5. Read/Write Contention (Mixed Workload):
 *    - WINNER: `HighContentionEventSource` (Abseil) and `RcuSource` again show their strength,
 * providing the best balance for mixed read/write workloads under contention.
 *
 * --- Raw Benchmark Data ---
 *
 ...
 * EventSourceFixture<HighContentionEventSource>/ReadWriteContention/4096/real_time/threads:16
 * 119826 ns EventSourceFixture<CallbackEventSource>/ReadWriteContention/4096/real_time/threads:16
 * 1020974 ns
 *
 * ======================================================================================================================
 */
...
#include "goldfish/eventing/policy_event_support.h"
...
template <typename T>
struct is_weak_ptr : std::false_type {};  // NOLINT

template <typename T>
struct is_weak_ptr<std::weak_ptr<T>> : std::true_type {};  // NOLINT

template <typename T>
inline constexpr bool is_weak_ptr_v = is_weak_ptr<T>::value;  // NOLINT
...
BENCHMARK_TEMPLATE_METHOD_F(EventSourceFixture, WriteContention)(benchmark::State& state) {
    const long n = state.range(0);
    for (auto _ : state) {
        const long i = (static_cast<long>(state.thread_index()) * 10) % n;
        this->source.AddListener(this->GetListener(i));
