 ...
 *
 * This function cycles through Red, Green, and Blue, allowing tests to use
...
class FillColorStrategy : public ImageGenerationStrategy {
  public:
    void generate(::pixman_image_t* target_image, int frame) override {
        int width = pixman_image_get_width(target_image);
        int height = pixman_image_get_height(target_image);
        uint32_t color = getFrameColor(frame);

        pixman_color_t fill_color = {static_cast<uint16_t>(((color >> 16) & 0xff) * 0x101),
    ...
    bool isGeneratedBy(::pixman_image_t* image, int frame) override {
        ...
        uint32_t expected_color = getFrameColor(frame);
...
class ChessboardStrategy : public ImageGenerationStrategy {
  public:
    void generate(::pixman_image_t* target_image, int frame) override {
        int width = pixman_image_get_width(target_image);
        int height = pixman_image_get_height(target_image);
        int square_size_w = width / 8;
        int square_size_h = height / 8;

        for (int y = 0; y < 8; ++y) {
            for (int x = 0; x < 8; ++x) {
                uint32_t color_val = ((x % 2) == (y % 2)) ? kColorWhite : kColorDarkGray;
    ...
    bool isGeneratedBy(::pixman_image_t* image, int frame) override {
        ...
        int stride = pixman_image_get_stride(image) / sizeof(uint32_t);
        ...
        const double kWhiteLuminance = get_luminance(kColorWhite);
        const double kGrayLuminance = get_luminance(kColorDarkGray);
        const double kLuminanceThreshold = (kWhiteLuminance + kGrayLuminance) / 2.0;

        // Validate Vertical Bands
        std::vector<double> col_avg_lum(8, 0.0);
        int band_width = width / 8;
        if (band_width == 0) return false;

        for (int band = 0; band < 8; ++band) {
            double total_lum = 0;
            int pixel_count = 0;
            int start_col = band * band_width;
            int end_col = (band == 7) ? width : (band + 1) * band_width;
            for (int r = 0; r < height; ++r) {
                for (int c = start_col; c < end_col; ++c) {
                    total_lum += get_luminance(data[r * stride + c]);
                    pixel_count++;
                }
            }
        ...
        for (int i = 0; i < 8; ++i) {
            if ((i % 2) == 0) {  // Even bands should be light
                if (col_avg_lum[i] < kLuminanceThreshold) return false;
        ...
        int band_height = height / 8;
        if (band_height == 0) return false;

        for (int band = 0; band < 8; ++band) {
            double total_lum = 0;
            int pixel_count = 0;
            int start_row = band * band_height;
            int end_row = (band == 7) ? height : (band + 1) * band_height;
        ...
        for (int i = 0; i < 8; ++i) {
            if ((i % 2) == 0) {  // Even bands should be light
                if (row_avg_lum[i] < kLuminanceThreshold) return false;
            } else {  // Odd bands should be dark
                if (row_avg_lum[i] > kLuminanceThreshold) return false;
            }
        }
...
class LinearGradientStrategy : public ImageGenerationStrategy {
  public:
    void generate(::pixman_image_t* target_image, int frame) override {
        ...
        int stride = pixman_image_get_stride(target_image) / sizeof(uint32_t);

        uint32_t end_color = getFrameColor(frame);
        uint8_t r_end = (end_color >> 16) & 0xff;
        uint8_t g_end = (end_color >> 8) & 0xff;
        uint8_t b_end = (end_color >> 0) & 0xff;

        for (int x = 0; x < width; ++x) {
            double ratio = static_cast<double>(x) / (width - 1);
            uint8_t r = static_cast<uint8_t>(ratio * r_end);
            uint8_t g = static_cast<uint8_t>(ratio * g_end);
            uint8_t b = static_cast<uint8_t>(ratio * b_end);
            uint32_t pixel_color = (0xff << 24) | (r << 16) | (g << 8) | b;
            for (int y = 0; y < height; ++y) {
                data[y * stride + x] = pixel_color;
            }
        }
    }
    ...
    bool isGeneratedBy(::pixman_image_t* image, int frame) override {
        ...
        for (int x = 0; x < width; ++x) {
            ...
            uint8_t g = static_cast<uint8_t>(ratio * g_end);
            uint8_t b = static_cast<uint8_t>(ratio * b_end);
            uint32_t expected_color = (0xff << 24) | (r << 16) | (g << 8) | b;

            for (int y = 0; y < height; ++y) {
                if (data[y * stride + x] != expected_color) {
                    return false;
                }
            }
        }
