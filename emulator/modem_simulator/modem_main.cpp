...
void set_signal_strength_profile(int quality) {
    ...
    quality = 25 * quality;  // emulator signal from 0 to 4, scale up to 100
...
static void* s_notify_user_data = nullptr;           // Some opaque data to give the function
...
void start_calling_thread(std::string ss) {
    ...
    s_current_call_monitor_sock = cuttlefish::SharedFD::SocketLocalClient(s_host_server_port);
...
void process_msgs() {
    ...
    while (true) {
        ...
        } else if (msg.type == MODEM_MSG_ENDCALL || msg.type == MODEM_MSG_UPDATECALL) {
            if (s_current_call_monitor_sock->IsOpen()) {
                s_current_call_monitor_sock->Write(msg.sdata.data(), msg.sdata.size());
            }
        } else if (msg.type == MODEM_MSG_CTEC || msg.type == MODEM_MSG_SIGNAL ||
                   msg.type == MODEM_MSG_DATA_REG || msg.type == MODEM_MSG_TIME_UPDATE ||
                   msg.type == MODEM_MSG_RADIO_STATE || msg.type == MODEM_MSG_VOICE_REG ||
...
int stop_android_modem_simulator() {
    if (s_host_server_port > 0 && !modem_host_servers.empty()) {
        ...
        if (success == false) {
            ...
            DD("successfully saved nvram json file %s", nvram_config_file.c_str());
        }
...
int start_android_modem_simulator_detached(int modem_simulator_port, bool& isIpv4,
                                           const std::string& timezone, const char* phone_number) {
    ...
    int actual_guest_server_port = start_android_modem_guest_server(modem_simulator_port, isIpv4);
    {
        ...
        for (auto& fd : server_fds) {
            ...
            if (phone_number) modem_simulator->SetPhoneNumber(phone_number);
    ...
    DD("starring modem simulator ready at guest port %d host port %d", actual_guest_server_port,
       s_host_server_port);
