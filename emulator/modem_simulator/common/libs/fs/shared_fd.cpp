...
ssize_t SharedFD::WriteAll(const std::string& buffer) {
    bool success = android::base::socketSendAll(value_->fd_, buffer.data(), buffer.size());
    if (success) {
        return buffer.size();
    } else {
        return 0;
...
namespace {
int UnixSelect(SharedFDSet* read_set, SharedFDSet* write_set, SharedFDSet* error_set,
    ...
    int rval = TEMP_FAILURE_RETRY(::select(max_index, &readfds, &writefds, &errorfds, timeout));
...
int WindowsPollFDs(SharedFDSet& read_set, struct timeval* timeout) {
    ...
    int ret = WaitForMultipleObjects(myevents.size(), myevents.data(), FALSE, mytimeout);
    ...
    if (ret >= WAIT_OBJECT_0 && ret < WAIT_OBJECT_0 + myevents.size()) {
        ...
        for (int i = index + 1; i < myevents.size(); ++i) {
            ...
            if (ret == WAIT_OBJECT_0) {
                ...
                read_set.Set(myreadmap[fd]);
                ++numFdReady;
                VERBOSE_INFO(modem, "additional fd fired %d total fired fd: %d", fd, numFdReady);
            }
        }
    }
...
int Select(SharedFDSet* read_set, SharedFDSet* write_set, SharedFDSet* error_set,
    ...
    if (write_set || error_set) {
        // just ignore them
        LOG(WARNING) << "The modem simulator does not support write and error, ignoring them";
    }
...
SharedFD SharedFD::SocketLocalClient(const std::string& name, bool abstract, int in_type) {
    ...
    if (name.size() != prefix.size() + 4 || name.find(prefix) != 0 || in_type != SOCK_STREAM ||
        !abstract) {
        return SharedFD();
    }
