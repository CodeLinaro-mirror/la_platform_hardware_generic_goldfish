...
/** MESSAGE TEXT
 **/
/* convert a quoted message text into a utf8 string. Note: you can use 'str' as the destination
 * buffer with the current implementation. always return the number of utf8 bytes corresponding to
 * the original message string, even if utf8 is NULL and utf8len is 0
 */
...
/* get the message embedded in a SMS PDU as a utf8 byte array, returns the length of the message in
 * bytes */
...
extern int sms_receiver_get_text_message(SmsReceiver rec, int index, unsigned char* utf8,
                                         int utf8len);
