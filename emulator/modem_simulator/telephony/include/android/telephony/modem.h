...
extern AModem amodem_create(int base_port, AModemUnsolFunc unsol_func, int sim_present,
                            void* unsol_opaque);
...
extern void amodem_set_notification_callback(AModem modem, ModemCallback* callback_func,
                                             void* user_data);
 ...
 **/
typedef enum {
    A_RADIO_STATE_OFF = 0, /* Radio explicitly powered off (eg CFUN=0) */
...
/* retrieve operator name into user-provided buffer. returns number of writes written, including
 * terminating zero */
extern int amodem_get_operator_name(AModem modem, ANameIndex index, char* buffer, int buffer_size);

/* reset one operator name from a user-provided buffer, set buffer_size to -1 for zero-terminated
 * strings */
extern void amodem_set_operator_name(AModem modem, ANameIndex index, const char* buffer,
                                     int buffer_size);
...
typedef enum { A_CALL_VOICE = 0, A_CALL_DATA, A_CALL_FAX, A_CALL_UNKNOWN = 9 } ACallMode;
