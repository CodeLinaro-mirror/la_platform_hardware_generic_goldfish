...
private:
  ...
  void insert(std::unordered_map<AllowGroup, AllowCache> &cache,
              AllowGroup color, std::string iss, std::string path) {
  ...
  bool isAllowed(AllowGroup color, std::string_view subView,
...
AllowList::fromJson(std::string_view jsonWithComments) {
