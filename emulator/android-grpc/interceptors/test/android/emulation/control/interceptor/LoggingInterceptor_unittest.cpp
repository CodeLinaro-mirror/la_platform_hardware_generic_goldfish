...
public:
  MOCK_METHOD(bool, QueryInterceptionHookPoint, (InterceptionHookPoints),
              (override));
  ...
  MOCK_METHOD((std::unique_ptr<ChannelInterface>), GetInterceptedChannel, (),
              (override));
...
TEST(LoggingInterceptor, LoggerForwardsTheCall) {
  ...
  auto interceptor =
      std::unique_ptr<Interceptor>(factory->CreateServerInterceptor(nullptr));
...
TEST(LoggingInterceptor, LoggerSnipsOutLongParameters) {
  ...
  EXPECT_EQ(record.response, "width: 123 height: 321 image: "
