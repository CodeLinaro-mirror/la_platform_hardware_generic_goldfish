...
class BasicTokenAuthenticator : public grpc::MetadataCredentialsPlugin {
  public:
    ...
    grpc::Status GetMetadata(grpc::string_ref service_url, grpc::string_ref method_name,
        ...
        metadata->insert(std::make_pair(android::emulation::control::BasicTokenAuth::DEFAULT_HEADER,
                                        mToken));
...
bool EmulatorGrpcClient::hasOpenChannel(bool tryConnect) {
    if (tryConnect) {
        ...
        DD("%s to emulator: %s, after %f us", connect ? "Connected" : "Not connected",
           mAddress.c_str(), time);
    }
...
EmulatorGrpcClient::EmulatorGrpcClient(Endpoint dest, InterceptorFactories interceptors)
    : mEndpoint(dest) {
    ...
    if (!dest.required_headers().empty()) {
        ...
        for (const auto& header : dest.required_headers()) {
            DD("Adding header: %s, %s", header.key().c_str(), header.value().c_str());
    ...
    mChannel = grpc::experimental::CreateCustomChannelWithInterceptors(address, call_creds, maxSize,
...
absl::StatusOr<std::unique_ptr<EmulatorGrpcClient>> EmulatorGrpcClient::loadFromProto(
        std::string_view pathToEndpointProto, InterceptorFactories factories) {
    // Read the existing address book.
    std::fstream input(PathUtils::asUnicodePath(pathToEndpointProto.data()).c_str(),
                       std::ios::in | std::ios::binary);
    Endpoint endpoint;
    if (!input) {
        LOG(ERROR) << "File " << pathToEndpointProto.data() << " not found";
        return nullptr;
    } else if (!endpoint.ParseFromIstream(&input)) {
        LOG(ERROR) << "File " << pathToEndpointProto.data() << " does not contain a valid endpoint";
        return nullptr;
    }

    return Builder().withEndpoint(endpoint).withInterceptors(std::move(factories)).build();
}
...
std::shared_ptr<EmulatorGrpcClient> EmulatorGrpcClient::me() {
    ...
    if (!sMe) {
        LOG(WARNING) << "Emulator client has not yet been configured.. Call configure "
                        "me first!";
    }
    return sMe;
}
...
Builder& Builder::withInterceptor(ClientInterceptorFactoryInterface* factory) {
    mFactories.push_back(std::unique_ptr<ClientInterceptorFactoryInterface>(factory));
    return *this;
}
...
Builder& Builder::withDiscoveryFile(std::string discovery_file) {
    ...
    if (!iniFile.hasKey("grpc.port")) {
        mStatus = absl::InvalidArgumentError("No grpc port defined in " + discovery_file);
        return *this;
    }
