...
public:
 ...
 grpc::Status GetMetadata(
   ...
   metadata->insert(std::make_pair(
       android::emulation::control::BasicTokenAuth::DEFAULT_HEADER, mToken));
...
EmulatorGrpcClient::loadFromProto(std::string_view pathToEndpointProto,
  ...
  if (!input) {
    ...
    LOG(ERROR) << "File " << pathToEndpointProto.data()
...
Builder& Builder::withDiscoveryFile(std::string discovery_file) {
  ...
  if (!iniFile.hasKey("grpc.port")) {
    mStatus =
        absl::InvalidArgumentError("No grpc port defined in " + discovery_file);
    return *this;
  }
