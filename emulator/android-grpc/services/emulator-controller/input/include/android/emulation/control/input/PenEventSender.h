...
class PenEventSender : public EventSender<PenEvent> {
  public:
    PenEventSender(IMultiDisplay* multidisplay, PointerEventDispatcher* dispatcher)
        : EventSender<PenEvent>(multidisplay), mDispatcher(dispatcher) {};
    ~PenEventSender() = default;

  protected:
    absl::Status doSend(IDisplay& display, const PenEvent& event) override;
