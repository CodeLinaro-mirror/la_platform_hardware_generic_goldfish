...
class TouchEventSender : public EventSender<TouchEvent> {
  public:
    TouchEventSender(IMultiDisplay* multidisplay, PointerEventDispatcher* dispatcher)
        : EventSender<TouchEvent>(multidisplay), mDispatcher(dispatcher) {};
    ~TouchEventSender() = default;

  protected:
    absl::Status doSend(IDisplay& display, const TouchEvent& touch) override;

  private:
    PointerEventDispatcher* mDispatcher;
};
