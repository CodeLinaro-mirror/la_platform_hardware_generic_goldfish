 ...
 */
extern int android_parse_options(int* pargc, char*** pargv, AndroidOptions* opt);

/* Parse |port_string| into |console_port| and |adb_port|. Error checking is
 * done and appropriate warning or error messages shown if needed. If the
 * function returns false the output parameter is not touched.
 */
extern bool android_parse_port_option(const char* port_string, int* console_port, int* adb_port);
 ...
 * done and the appropriate warning or error messages shown if needed. If the
 * function returns false neither of the output parameters are touched.
 */
extern bool android_parse_ports_option(const char* ports_string, int* console_port, int* adb_port);
