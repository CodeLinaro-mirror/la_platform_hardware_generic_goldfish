...
CFG_PARAM(monitor_adb, "<verbose_level>",
          "monitor the adb messages between guest and host, default not")
OPT_PARAM(snapstorage, "<file>",
          "file that contains all state snapshots (default <datadir>/snapshots.img)")
OPT_FLAG(no_snapstorage,
         "do not mount a snapshot storage file (this disables all snapshot functionality)")
OPT_PARAM(snapshot, "<name>",
          "name of snapshot within storage file for auto-start and auto-save (default "
          "'default-boot')")
OPT_FLAG(no_snapshot,
         "perform a full boot and do not auto-save, but qemu vmload and vmsave operate on "
         "snapstorage")
...
OPT_FLAG(force_snapshot_load,
         "force starting from snapshot: exit emulator if loading from snapshot fails")
...
OPT_PARAM(dpi_device, "<dpi>",
          "specify device's resolution in dpi (default " STRINGIFY(DEFAULT_DEVICE_DPI) ")")
...
OPT_PARAM(net_socket, "option",
          "use Socket interface for network, one of fd=, listen=, or connect=")
...
CFG_PARAM(gnss_grpc_port, "<port number>",
...
OPT_FLAG(read_only,
         "allow running multiple instances of emulators on the same"
         " AVD, but cannot save snapshot.")
OPT_PARAM(is_restart, "<restart-pid>",
          "specifies that this emulator was a restart, and to wait out <restart-pid> before "
          "proceeding")
...
OPT_LIST(virtualscene_poster, "<name>=<filename>",
...
OPT_PARAM(metrics_to_file, "<file>",
...
OPT_FLAG(fuchsia,
         "Run Fuchsia image. Bypasses android-specific setup; args after are treated as standard "
         "QEMU args")

OPT_PARAM(window_size, "<size>", "Set window size for when bypassing android-specific setup.")

OPT_FLAG(allow_host_audio,
...
OPT_FLAG(grpc_use_jwt,
         "Use a signed JWT token for gRPC authentication. (default, disable with -grpc flag)")
OPT_PARAM(grpc_allowlist, "<file>", "Use a custom allow list for gRPC authentication.")
OPT_PARAM(idle_grpc_timeout, "<timeout>",
...
OPT_PARAM(turncfg, "cmd",
          "Command to execute to obtain turn configuration for the webrtc connection.")
OPT_PARAM(dump_audio, "<file>", "Dump audio to apointed file for the webrtc connection.")
#endif

OPT_PARAM(packet_streamer_endpoint, "<uri>",
...
OPT_FLAG(use_keycode_forwarding, "Use keycode forwarding instead of host charmap translation.")
...
OPT_PARAM(check_snapshot_loadable, "<snapshot name|exported snapshot tar file>",
...
OPT_PARAM(wifi_socket, "<option>",
          "use Socket interface for Virtio Wi-Fi, one of fd=, listen=, or connect=")
#if defined(__APPLE__) && defined(__arm64__)
OPT_PARAM(vmnet_bridged, "<host network interface>",
          "Enable vmnet framework in bridged mode as the backend of tap netdev on MacOS and "
...
OPT_PARAM(vmnet_start_address, "<ipv4 address>",
          "If using vmnet in shared mode, starting IPv4 address to assign using DHCP. Must be in "
          "RFC 1918 private IP range and specified together with vmnet_end_address and "
          "vmnet_subnet_mask.")

OPT_PARAM(vmnet_end_address, "<ipv4 address>",
          "If using vmnet in shared mode, ending IPv4 address to assign using DHCP. Must be in RFC "
          "1918 private IP range and specified together with vmnet_start_address and "
          "vmnet_subnet_mask.")

OPT_PARAM(vmnet_subnet_mask, "<ipv4 address>",
          "If using vmnet in shared mode, IPv4 netmask to assign using DHCP. Must be in RFC 1918 "
          "private IP range and specified together with vmnet_start_address and vmnet_end_address.")

OPT_FLAG(vmnet_isolated,
         "Enable isolation for the interface. Ensures that vmnet interface is not able to "
         "communicate with any other vmnet interface.")
#endif
OPT_PARAM(wifi_user_mode_options, "<option list>",
          "Override default user mode networking option for wifi network in "
          "Android Emulator for API 31 and above.")

OPT_PARAM(network_user_mode_options, "<option list>",
...
OPT_PARAM(wifi_mac_address, "<mac address>",
...
OPT_PARAM(android_serialno, "<android serialno>",
...
OPT_FLAG(no_netsim, "disable launching netsimd and configuring any devices that depend on it e.g. wifi, bt, uwb")
