...
bool android_network_speed_parse(const char* speed, double* upload_bauds, double* download_bauds) {
    if (!speed || !speed[0]) {
...
bool android_network_latency_parse(const char* delay, double* min_delay_ms, double* max_delay_ms) {
    if (!delay || !delay[0]) {
