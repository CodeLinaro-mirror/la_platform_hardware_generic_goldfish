...
TEST(CmdLineOptions, parsePort) {
    ...
    for (const auto& data : kData) {
        int consolePort = 0;
        int adbPort = 0;
        bool result = android_parse_port_option(data.input, &consolePort, &adbPort);
        EXPECT_EQ(data.expectSuccess, result) << "Failed on test case: (" << data.input << ")";
        EXPECT_EQ(data.expectConsolePort, consolePort)
                << "Failed on test case: (" << data.input << ")";
        EXPECT_EQ(data.expectAdbPort, adbPort) << "Failed on test case: (" << data.input << ")";
    }
}
...
TEST(CmdLineOptions, parsePorts) {
    ...
    for (const auto& data : kData) {
        ...
        bool result = android_parse_ports_option(data.input, &consolePort, &adbPort);
        EXPECT_EQ(data.expectSuccess, result) << "Failed on test case: (" << data.input << ")";
        EXPECT_EQ(data.expectConsolePort, consolePort)
                << "Failed on test case: (" << data.input << ")";
        EXPECT_EQ(data.expectAdbPort, adbPort) << "Failed on test case: (" << data.input << ")";
...
TEST(CmdLineOptions, validatePorts) {
    ...
    for (const auto& data : kData) {
        bool result = android_validate_ports(data.consolePortInput, data.adbPortInput);
        EXPECT_EQ(data.expectSuccess, result) << "Failed on test case: (" << data.consolePortInput
                                              << "," << data.adbPortInput << ")";
    }
}
