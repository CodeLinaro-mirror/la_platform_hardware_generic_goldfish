...
TEST(AndroidNetworkConstants, ParseSpeed) {
    ...
    for (size_t n = 0; n < ARRAYLEN(kData); ++n) {
        double upload = 0.;
        double download = 0.;
        bool result = android_network_speed_parse(kData[n].input, &upload, &download);
...
TEST(AndroidNetworkConstants, ParseSpeedWithName) {
    for (size_t n = 0; n < android_network_speeds_count; ++n) {
        ...
        double upload = 0.;
        double download = 0.;
        EXPECT_TRUE(android_network_speed_parse(entry->name, &upload, &download));
...
TEST(AndroidNetworkConstants, ParseLatency) {
    ...
    for (size_t n = 0; n < ARRAYLEN(kData); ++n) {
        double min_ms = 0.;
        double max_ms = 0.;
        bool result = android_network_latency_parse(kData[n].input, &min_ms, &max_ms);
        EXPECT_EQ(kData[n].expected_result, result) << "# " << n;
        if (!result) {
            continue;
        }
...
TEST(AndroidNetworkConstants, ParseLatencyWithName) {
    for (size_t n = 0; n < android_network_latencies_count; ++n) {
        ...
        double min_ms = 0.;
        double max_ms = 0.;
        EXPECT_TRUE(android_network_latency_parse(entry->name, &min_ms, &max_ms));
