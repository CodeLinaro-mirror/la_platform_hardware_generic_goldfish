...
#include "emulator/plugin/netsim/h4_parser.h"  // for H4Parser, PacketType, H4Pars...
...
void H4Parser::OnPacketReady() {
  switch (hci_packet_type_) {
    ...
    case PacketType::ISO:
      ...
      break;
    default:
      LOG(WARNING) << absl::StrFormat("Unimplemented packet type %d", static_cast<int>(hci_packet_type_));
      std::abort();
  }
...
bool H4Parser::Consume(const uint8_t* buffer, int32_t bytes_read) {
  ...
  if ((uint32_t)bytes_read > BytesRequested()) {
    LOG(WARNING) << absl::StrFormat("More bytes read (%u) than expected (%u)!", static_cast<int>(bytes_read), static_cast<int>(bytes_to_read));
    std::abort();
  }
  ...
  switch (state_) {
    ...
    case HCI_RECOVERY: {
      ...
      LOG(WARNING) << absl::StrFormat("Received byte in recovery state : 0x%x", static_cast<unsigned>(*buffer));
      ...
      if (packet_.size() == reset_command.size()) {
        LOG(WARNING) << "Received HCI Reset command, exiting recovery state";
    ...
    case HCI_TYPE:
      ...
      if (hci_packet_type_ != PacketType::ACL &&
        ...
        if (!enable_recovery_state_) {
          LOG(WARNING) << absl::StrFormat("Received invalid packet type 0x%x", static_cast<unsigned>(packet_type_));
          std::abort();
        }
        LOG(ERROR) << absl::StrFormat("Received invalid packet type 0x%x, entering recovery state", static_cast<unsigned>(packet_type_));
        state_ = HCI_RECOVERY;
        hci_packet_type_ = PacketType::COMMAND;
        bytes_wanted_ = 1;
      } else {
