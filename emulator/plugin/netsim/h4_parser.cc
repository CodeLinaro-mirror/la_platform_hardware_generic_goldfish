...
void H4Parser::OnPacketReady() {
    ...
    case PacketType::ISO:
        ...
        break;
    default:
        LOG(WARNING) << absl::StrFormat("Unimplemented packet type %d",
...
bool H4Parser::Consume(const uint8_t* buffer, int32_t bytes_read) {
    ...
    if ((uint32_t)bytes_read > BytesRequested()) {
        LOG(WARNING) << absl::StrFormat("More bytes read (%u) than expected (%u)!",
    ...
    case HCI_RECOVERY: {
        ...
        LOG(WARNING) << absl::StrFormat("Received byte in recovery state : 0x%x",
    ...
    case HCI_TYPE:
        ...
        if (hci_packet_type_ != PacketType::ACL && hci_packet_type_ != PacketType::SCO &&
            ...
            if (!enable_recovery_state_) {
                LOG(WARNING) << absl::StrFormat("Received invalid packet type 0x%x",
                                                static_cast<unsigned>(packet_type_));
                std::abort();
            }
            LOG(ERROR) << absl::StrFormat(
                    "Received invalid packet type 0x%x, entering recovery state",
                    static_cast<unsigned>(packet_type_));
            state_ = HCI_RECOVERY;
