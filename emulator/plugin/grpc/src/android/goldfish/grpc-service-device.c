...
static void grpc_set_port(Object *obj, Visitor *v, const char *name,
  ...
  if (value < 0 || value > 65535) {
    error_setg(errp, "Port number should be between 0 and 65535, not: %d",
               value);
    return;
  }
...
static void grpc_set_idle_timeout(Object *obj, Visitor *v, const char *name,
                                  void *opaque, Error **errp) {
  GrpcDev *grpc_device = GRPC_DEV(obj);
  uint32_t value;
...
static void grpc_class_init(ObjectClass *oc, void *data) {
  ...
  object_class_property_set_description(
      oc, "tls_cer", "PEM file with a X.509 public key certificate.");

  object_class_property_add_str(oc, "tls_key", NULL, grpc_set_tls_key);
  object_class_property_set_description(
      oc, "tls_key", "PEM file containing a private key used for TLS.");
  ...
  object_class_property_set_description(
      oc, "allowlist", "A json file describing the access rules.");

  object_class_property_add_str(oc, "addr", NULL, grpc_set_addr);
  object_class_property_set_description(
      oc, "addr", "Address to which the grpc service should be bound.");

  object_class_property_add_str(oc, "avd", NULL, grpc_set_avd);
  object_class_property_set_description(
      oc, "avd",
      "name of one of the existing AVDs available on your host machine");
  object_class_property_add(oc, "port", "int", NULL, grpc_set_port, NULL, NULL);
  object_class_property_set_description(
      oc, "port", "The port to which the grpc service should be bound.");

  object_class_property_add(oc, "idle_timeout", "int", NULL,
                            grpc_set_idle_timeout, NULL, NULL);
  object_class_property_set_description(
      oc, "idle_timeout",
