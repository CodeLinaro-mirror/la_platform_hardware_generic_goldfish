...
static void copy_pixman_image_to_buffer(pixman_image_t *image, uint8_t *pixels,
                                        size_t cPixels) {
...
static absl::Status copy_buffer_to_pixman_image(pixman_image_t *src_img,
  ...
  dst_img = pixman_image_create_bits(pixmanFormat(format), format.width(),
                                     format.height(), NULL, 0);

  // Set up the transformation
  pixman_transform_init_scale(
      &transform, (double)format.width() / pixman_image_get_width(src_img),
...
Image QemuDisplayTransformer::getScreenshot(ImageFormat fmt) {
  ...
  if (renderer->getScreenshot(nChannels, &width, &height, nullptr, &cPixels,
                              fmt.display(), fmt.width(), fmt.height(),
                              0) != 0) {
    ...
    char *unsafe = image.mutable_image()->data();
    uint8_t *pixels = reinterpret_cast<uint8_t *>(unsafe);
    renderer->getScreenshot(nChannels, &width, &height, pixels, &cPixels,
                            fmt.display(), fmt.width(), fmt.height(), 0);
  }

  image.mutable_format()->CopyFrom(fmt);
  LOG(INFO) << "QemuDisplayTransformer::getScreenshot: "
            << image.ShortDebugString();
  return image;
}

ImageEventSupport *QemuDisplayTransformer::addListener(ImageFormat fmt) {
  const std::lock_guard<std::mutex> lock(mListenerLock);
  LOG(INFO) << "QemuDisplayTransformer::addListener: "
            << fmt.ShortDebugString();
  if (!mListenerMap.count(fmt)) {
    ...
    mListenerMap[fmt] =
        std::make_tuple<std::unique_ptr<ImageEventSupport>, Image>(
  ...
  LOG(ERROR) << "Failed to find listener for format: "
             << fmt.ShortDebugString();
  return nullptr;
}
...
void QemuDisplayTransformer::fireEvent(pixman_image_t *src_img) {
  ...
  for (auto &[format, image_pair] : mListenerMap) {
    ...
    if (!status.ok()) {
      LOG(ERROR) << "Failed to copy buffer to pixman image, skipping. "
                 << status.message();
      continue;
    }

    LOG(INFO) << "QemuDisplayTransformer::fireEvent: "
              << format.ShortDebugString();
