...
EmulatorAdvertisement::~EmulatorAdvertisement() { garbageCollect(); }
...
void EmulatorAdvertisement::remove() const {
  ...
  auto pid_dir =
      android::base::pj(mSharedDirectory, std::to_string(Process::me()->pid()));
...
bool EmulatorAdvertisement::exists(base::Pid pid) {
  std::string pidfile = android::base::StringFormat(location_format, pid);
  std::string pidPath =
      android::base::pj(goldfish::ConfigDirs::getDiscoveryDirectory(), pidfile);
