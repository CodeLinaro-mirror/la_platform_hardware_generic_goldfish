...
#define GOLDFISH_BATTERY(obj) \
    OBJECT_CHECK(struct goldfish_battery_state, (obj), TYPE_GOLDFISH_BATTERY)
...
int goldfish_battery_read_prop(int property) {
    ...
    DeviceState* dev = qdev_find_recursive(sysbus_get_default(), TYPE_GOLDFISH_BATTERY);
...
void goldfish_battery_set_prop(int ac, int property, int value) {
    ...
    if (new_status != battery_state->int_status) {
        ...
        safe_set_irq(battery_state->irq, (battery_state->int_status & battery_state->int_enable));
    }
}
...
static void goldfish_battery_write(void* opaque, hwaddr offset, uint64_t val, unsigned size) {
    struct goldfish_battery_state* s = opaque;
...
static void goldfish_battery_realize(DeviceState* dev, Error** errp) {
    ...
    memory_region_init_io(&s->iomem, OBJECT(s), &goldfish_battery_iomem_ops, s, "goldfish_battery",
                          0x1000);
