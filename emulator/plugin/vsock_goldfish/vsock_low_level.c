...
static void virtqueue_consume_elem(VirtQueue *const vq,
                                   VirtQueueElement *const e,
...
static VirtIOVSockSendResult
virtio_vsock_send_packet_host_to_guest(void *const s_raw,
                                       struct virtio_vsock_hdr *const hdr,
    ...
    do {
        ...
        const size_t data8_chunk_size =
            MIN(data8_size, e_size - sizeof(struct virtio_vsock_hdr));

        hdr->len = data8_chunk_size;
        iov_from_buf(e->in_sg, e->in_num, 0, hdr, sizeof(*hdr));
        iov_from_buf(e->in_sg, e->in_num, sizeof(*hdr), data8,
                     data8_chunk_size);
...
static void virtio_vsock_parse_guest_to_host(VirtIOVSock *const s,
                                             void *impl,
...
static void virtio_vsock_handle_guest_to_host(VirtIODevice *const dev,
                                              VirtQueue *const vq) {
...
static VirtIOVSockSendResult
virtio_vsock_send_event_host_to_guest(void *const s_raw,
                                      const struct virtio_vsock_event *const event) {
    bool need_notify = false;
    bool is_vq_full = false;
...
static void virtio_vsock_device_realize(DeviceState *const dev, Error **errp) {
    ...
    if ((s->guest_cid <= VMADDR_CID_HOST) ||
            (s->guest_cid == UINT32_MAX) ||
            (s->guest_cid == UINT64_MAX)) {
        error_setg(errp, "The '%s' property value (%" PRIu64 ") is incorrect",
                   "guest-cid", s->guest_cid);
        return;
    }
    ...
    s->host_to_guest_vq =
        virtio_add_queue(v, VIRTIO_VSOCK_QUEUE_SIZE,
                         &virtio_vsock_handle_host_to_guest);
    s->guest_to_host_vq =
        virtio_add_queue(v, VIRTIO_VSOCK_QUEUE_SIZE,
                         &virtio_vsock_handle_guest_to_host);
    s->host_to_guest_event_vq =
        virtio_add_queue(v, VIRTIO_VSOCK_EVENT_QUEUE_SIZE,
                         &virtio_vsock_handle_event_to_guest);
...
static uint64_t virtio_vsock_device_get_features(VirtIODevice *const dev,
                                                 const uint64_t requested_features,
                                                 Error **errp) {
...
static void virtio_vsock_set_status(VirtIODevice *const dev,
...
static int vmstate_info_virtio_vsock_impl_load(QEMUFile *const f,
                                               void *const opaque,
                                               size_t size,
...
static int vmstate_info_virtio_vsock_impl_save(QEMUFile *const f,
                                               void *const opaque,
                                               size_t size,
                                               const VMStateField *field,
                                               JSONWriter *json) {
