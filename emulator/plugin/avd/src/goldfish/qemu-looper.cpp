  ...
  public:
    ...
    class FdWatch : public BaseFdWatch {
      public:
        ...
        virtual unsigned poll() const { return mPendingEvents; }

        bool isPending() const { return mPendingEvents != 0U; }
      ...
      private:
        void updateEvents(unsigned events) {
            ...
            qemu_set_fd_handler(mFd, cbRead, cbWrite, (cbRead || cbWrite) ? this : nullptr);
    ...
    virtual BaseFdWatch* createFdWatch(int fd, BaseFdWatch::Callback callback,
    ...
    class Task : public BaseLooper::Task {
      public:
        ...
        void schedule() override { qemu_bh_schedule(mBottomHalf); }

        void cancel() override { qemu_bh_cancel(mBottomHalf); }

      protected:
        static void handleBottomHalf(void* opaque) { static_cast<Task*>(opaque)->run(); }

        virtual void run() { mCallback(); }
    ...
    virtual void forceQuit() override {
        LOG(INFO) << "Qemu looper has been forced to quit, shutting down qemu.";
