...
#include "hardware/generic/goldfish/emulator/grpc/services/emulator-controller/proto/emulator_controller.pb.h"
...
TEST(LoggingInterceptor, LoggerRecordsData) {
    ...
    EXPECT_CALL(batchMethods, Proceed());
    EXPECT_CALL(batchMethods, GetSendStatus()).WillOnce(Return(Status::CANCELLED));
