...
constexpr std::string_view kSysfsAndroidDtDirDtb =
...
std::vector<std::pair<std::string, std::string>> getUserspaceBootProperties(
    std::string targetArch, std::string serialno,
  ...
  std::string qemuScreenOffTimeoutProp =
  ...
  std::string qemuDrawFlushIntervalProp =
  ...
  std::string qemuCameraProtocolVerProp =
      "androidboot.qemu.camera_protocol_ver";
  std::string qemuCameraHqEdgeProp =
      "androidboot.qemu.camera_hq_edge_processing";
  std::string qemuDisplaySettingsXmlProp =
      "androidboot.qemu.display.settings.xml";
  ...
  params.push_back({qemuOpenglesVersionProp,
                    absl::StrFormat("%d", bootPropOpenglesVersion)});
  ...
  if (hw.vm_heapSize > 0) {
    params.push_back(
        {dalvikVmHeapsizeProp, absl::StrFormat("%dm", hw.vm_heapSize)});
  }
  ...
  if (isX86ish) {
    ...
    params.push_back(
        {"androidboot.android_dt_dir", std::string(kSysfsAndroidDtDir)});
  }
  ...
  if (hasShellConsole) {
    params.push_back(
        {"androidboot.console", absl::StrFormat("%s0", kernelSerialPrefix)});
  }
...
static std::string getDynamicPartitionBootDevice(const Emulator& emulator) {
  ...
  if (drive) {
    return absl::StrFormat("/dev/block/platform/%s.virtio_mmio/by-name/%s",
                           drive->addr(), "system");
  }
