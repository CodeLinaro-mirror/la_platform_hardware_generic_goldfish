...
//     MockAvd* avd_ptr = avd.get();
    ...
    EXPECT_CALL(mock_avd, getSystemImageFilePath(Avd::ImageType::INITSYSTEM))
            .WillRepeatedly(
                    Return(system_dir / "system.img"));
    EXPECT_CALL(mock_avd, getSystemImageFilePath(Avd::ImageType::INITVENDOR))
            .WillRepeatedly(
                    Return(system_dir / "vendor.img"));
    EXPECT_CALL(mock_avd,
                getSystemImageFilePath(Avd::ImageType::ENCRYPTIONKEY))
...
//             .WillRepeatedly(Return(absl::NotFoundError("")));
    EXPECT_CALL(mock_avd, getContentPath())
            .WillRepeatedly(Return(user_dir));

    EXPECT_CALL(mock_container,
                addRoDrive("system", "03.0",
                           system_dir / "system.img"))
            .Times(1);
    EXPECT_CALL(mock_container, addRwDrive("encrypt", "06.0", _, _, _, _, _))
            .Times(1);
    EXPECT_CALL(mock_container,
                addRoDrive("vendor", "07.0",
                           system_dir / "vendor.img"))
            .Times(1);
    EXPECT_CALL(mock_container, addRwDrive("userdata", "05.0", _, _, _, _, _))
            .Times(1);
