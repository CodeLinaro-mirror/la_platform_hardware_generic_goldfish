...
static void help_list_avds(stralloc_t* out) {
    PRINTF("  List all available AVDs\n\n"
           ...
           "  Note that the home directory can be overridden by defining the\n"
           "  ANDROID_SDK_HOME environment variable.\n\n");
}

static void help_studio_params(stralloc_t* out) {
    PRINTF("  Indicates a temporary file containing parameters from Android Studio.\n\n"
...
static void help_build_images(stralloc_t* out) {
    PRINTF("  The emulator detects that you are working from the Android build system\n"
           ...
           "  You can use the -sysdir, -system, -vendor -kernel, -ramdisk, -datadir, -data "
           "options\n"
...
static void help_disk_images(stralloc_t* out) {
    PRINTF("  The emulator needs several key image files to run appropriately.\n"
           ...
           "  can still run the emulator by explicitly providing the paths to\n"
...
static void help_avd(stralloc_t* out) {
    PRINTF("  use '-avd <name>' to start the emulator program with a given Android\n"
...
static void help_avd_arch(stralloc_t* out) {
    PRINTF("  use '-avd-arch <target>' to start the emulator program with a specific\n"
...
static void help_kernel(stralloc_t* out) {
    PRINTF("  use '-kernel <file>' to specify a Linux kernel image to be run.\n"
...
static void help_ramdisk(stralloc_t* out) {
    PRINTF("  use '-ramdisk <file>' to specify a Linux ramdisk boot image to be run in\n"
           "  the emulator. the default image is 'ramdisk.img' from the system directory.\n\n"

           "  see '-help-disk-images' for more information about disk image files\n\n");
}

static void help_logcat_output(stralloc_t* out) {
    PRINTF("  use '-logcat-output <file>' to specify a path to save logcat output\n\n");
}

static void help_system(stralloc_t* out) {
    PRINTF("  use '-system <file>' to specify the initial system image that will be loaded.\n"
...
static void help_vendor(stralloc_t* out) {
    PRINTF("  use '-vendor <file>' to specify the initial system image that will be loaded.\n"
           "  the default image is 'vendor.img' from the system directory.\n\n"

           "  see '-help-disk-images' for more information about disk image files\n\n");
}

static void help_encryption_key(stralloc_t* out) {
    PRINTF("  use '-encryption-key <file>' to specify the initial encryption key image that will "
           "be "
           "loaded.\n"
...
static void help_image(stralloc_t* out) {
    PRINTF("  This option is obsolete, you should use '-system <file>' instead to point\n"
           "  to the initial system image.\n\n"

           "  see '-help-disk-images' for more information about disk image files\n\n");
}

static void help_data(stralloc_t* out) {
    PRINTF("  use '-data <file>' to specify a different /data partition image file.\n\n"

           "  see '-help-disk-images' for more information about disk image files\n\n");
}

static void help_wipe_data(stralloc_t* out) {
    PRINTF("  use '-wipe-data' to reset your /data partition image to its factory\n"
           "  defaults. this removes all installed applications and settings.\n\n"

           "  see '-help-disk-images' for more information about disk image files\n\n");
}

static void help_writable_system(stralloc_t* out) {
    PRINTF("  use -writable-system to have a writable system image during your\n"
...
static void help_cache(stralloc_t* out) {
    PRINTF("  use '-cache <file>' to specify a /cache partition image. if <file> does\n"
...
static void help_cache_size(stralloc_t* out) {
    PRINTF("  use '-cache <size>' to specify a /cache partition size in MB. By default,\n"
           "  the cache partition size is set to 66MB\n\n");
}

static void help_no_cache(stralloc_t* out) {
    PRINTF("  use '-no-cache' to disable the cache partition in the emulated system.\n"
...
static void help_sdcard(stralloc_t* out) {
    PRINTF("  use '-sdcard <file>' to specify a SD Card image file that will be attached\n"
...
static void help_snapstorage(stralloc_t* out) {
    PRINTF("  Use '-snapstorage <file>' to specify a repository file for snapshots.\n"
...
static void help_no_snapstorage(stralloc_t* out) {
    PRINTF("  This starts the emulator without mounting a file to store or load state\n"
...
static void help_snapshot(stralloc_t* out) {
    PRINTF("  Rather than executing a full boot sequence, the Android emulator can\n"
...
static void help_no_snapshot(stralloc_t* out) {
    PRINTF("  This inhibits both the autoload and autosave operations, forcing\n"
...
static void help_no_snapshot_load(stralloc_t* out) {
    PRINTF("  Prevents the emulator from loading the AVD's state from the snapshot\n"
           "  storage on start.\n\n");
}

static void help_force_snapshot_load(stralloc_t* out) {
    PRINTF("  Forces the emulator to load the AVD's state from a snapshot, meaning\n"
...
static void help_no_snapshot_save(stralloc_t* out) {
    PRINTF("  Prevents the emulator from saving the AVD's state to the snapshot\n"
           "  storage on exit, meaning that all changes will be lost.\n\n");
}

static void help_no_snapshot_update_time(stralloc_t* out) {
    PRINTF("  Prevent the emulator from sending an unsolicited time update\n"
...
static void help_snapshot_list(stralloc_t* out) {
    PRINTF("  This prints a table of snapshots that are stored in the snapshot storage\n"
...
static void help_accel(stralloc_t* out) {
    PRINTF("  Use '-accel <mode>' to control how CPU emulation can be accelerated\n"
...
static void help_no_accel(stralloc_t* out) {
    PRINTF("  Use '-no-accel' as a shortcut to '-accel off', i.e. to disable accelerated\n"
           "  CPU emulation, when emulating an x86 or x86_64 system image. Only useful\n"
           "  for debugging.\n");
}

static void help_ranchu(stralloc_t* out) {
    PRINTF("  DEPRECATED: Previously used to use the new emulator engine as the\n"
...
static void help_engine(stralloc_t* out) {
    PRINTF("  Select the emulation engine to use. Valid values are\n\n"
...
static void help_skindir(stralloc_t* out) {
    PRINTF("  use '-skindir <dir>' to specify a directory that will be used to search\n"
...
static void help_skin(stralloc_t* out) {
    PRINTF("  use '-skin <skin>' to specify an emulator skin, each skin corresponds to\n"
...
static void help_shaper(stralloc_t* out) {
    int n;
    PRINTF("  the Android emulator supports network throttling, i.e. slower network\n"
    ...
    for (n = 0; n < android_network_latencies_count; ++n) {
        const AndroidNetworkLatency* android_netdelay = &android_network_latencies[n];
        PRINTF("    -netdelay %-10s   %-15s  (min %d, max %d)\n", android_netdelay->name,
               android_netdelay->display_name, android_netdelay->min_ms, android_netdelay->max_ms);
    }
...
static void help_wifi_client_port(stralloc_t* out) {
    PRINTF("  the Android emulator allows forwarding of WiFi traffic from one emulator\n"
...
static void help_wifi_server_port(stralloc_t* out) {
    PRINTF("  the Android emulator allows forwarding of WiFi traffic from one emulator\n"
           "  to another emulator. This makes it appear as if the two emulators are on\n"
           "  the same WiFi network. In order to do this one emulator acts as a server\n"
           "  and the other emulators acts as a client. The client emulator connects\n"
...
static void help_http_proxy(stralloc_t* out) {
    PRINTF("  the Android emulator allows you to redirect all TCP connections through\n"
...
static void help_report_console(stralloc_t* out) {
    PRINTF("  the '-report-console <socket>' option can be used to report the\n"
...
static void help_ui_only(stralloc_t* out) {
    PRINTF("  the '-ui-only <UI feature>' option is used to run only that part\n"
...
static void help_id(stralloc_t* out) {
    PRINTF("  '-id <name>' specifies the ID of the virtual device\n"
...
static void help_dpi_device(stralloc_t* out) {
    PRINTF("  the '-dpi-device <dpi>' option is obsolete as of Emulator 2.0 and will be "
           "ignored\n\n");
}

static void help_audio(stralloc_t* out) {
    PRINTF("  the '-audio <backend>' option allows you to select a specific backend\n"
           "  to be used to both play and record audio in the Android emulator.\n\n"

           "  use '-audio none' to disable audio completely.\n\n");
}

static void help_scale(stralloc_t* out) {
    PRINTF("  the '-scale <scale>' option is obsolete as of Emulator 2.0 and will be ignored\n\n");
}

static void help_code_profile(stralloc_t* out) {
    PRINTF("  use '-code_profile <name>' to start the emulator with runtime code profiling "
           "support.\n\n"
...
static void help_show_kernel(stralloc_t* out) {
    PRINTF("  use '-show-kernel' to redirect debug messages from the kernel to the current\n"
           "  terminal. this is useful to check that the boot process works correctly.\n\n");
}

static void help_shell(stralloc_t* out) {
    PRINTF("  use '-shell' to create a root shell console on the current terminal.\n"
...
static void help_shell_serial(stralloc_t* out) {
    PRINTF("  use '-shell-serial <device>' instead of '-shell' to open a root shell\n"
...
static void help_logcat(stralloc_t* out) {
    PRINTF("  use '-logcat <tags>' to redirect log messages from the emulated system to\n"
...
static void help_no_audio(stralloc_t* out) {
    PRINTF("  use '-no-audio' to disable all audio support in the emulator. this may be\n"
...
static void help_radio(stralloc_t* out) {
    PRINTF("  use '-radio <device>' to redirect the GSM modem emulation to an external\n"
...
static void help_port(stralloc_t* out) {
    PRINTF("  at startup, the emulator tries to bind its control console at a free port\n"
...
static void help_ports(stralloc_t* out) {
    PRINTF("  the '-ports <consoleport>,<adbport>' option allows you to explicitly set\n"
...
static void help_log_detailed(stralloc_t* out) {
    PRINTF("  Enable detailed logging.\n\n");
}

static void help_grpc(stralloc_t* out) {
    PRINTF("  Enables the gRPC service to control the emulator.\n\n"
           "    <port> is the TCP port used to bind the gRPC service\n\n"
           "  Note: You will have to explicitly turn on jwt authorization with the -grpc-use-jwt "
           "flag\n\n"
           "  The gRPC service will not be started if the port is not available on startup.\n\n");
}

static void help_grpc_tls_key(stralloc_t* out) {
    PRINTF("  Use the PEM file containing a private key for TLS.\n\n"
           "    <pem> a PEM file containing a private key used for TLS.\n\n"
           "  You must provide the -grpc-tls-cer flag as well.\n\n");
}

static void help_grpc_tls_cer(stralloc_t* out) {
    PRINTF("  Use the given file containing a X509 certificate for TLS.\n\n"
...
static void help_grpc_tls_ca(stralloc_t* out) {
    PRINTF("  Use the file containing a series of certificate authorities for client "
           "validation.\n\n"
...
static void help_grpc_use_token(stralloc_t* out) {
    PRINTF("  Require an authorization header with a valid token for every grpc call.\n\n"
           "  Every grpc request expects the following header:\n\n"
           "    authorization: Bearer <token>\n\n"
           "  The token can be found in the android studio discovery file under the key: "
           "grpc.token.\n"
           ...
           "  The file will be named `pid_%%d_info.ini` where %%d is the process id of the "
           "emulator. \n\n"
           "  Note: Token based security can only be installed if you are using localhost or "
           "TLS.\n\n",
#ifdef __linux__
           "$XDG_RUNTIME_DIR"
#endif
...
static void help_grpc_use_jwt(stralloc_t* out) {
    PRINTF("  Require an authorization header with a valid jwt token for every grpc call.\n\n"
           "  Every grpc request expects the following header:\n\n"
           "    authorization: Bearer <token>\n\n"
           "  Where token is a valid signed JSON Web Token (JWT)\n\n"
           "  If an incorrect token is present the status UNAUTHORIZED will be returned.\n\n"
           "  In order to make a successful call a JWT token must be presented with the following "
           "fields:\n\n"
           "  - aud: must contain a list of allowed methods. \n"
           "  - iat: must have an issue in the past. \n"
           "  - exp: must have an expiration date. \n\n"
           "  In order to validate tokens the emulator will load all JSON Web Key (JWK) files "
           "from\n"
           "  a directory that is only accessible by the user who launched the emulator.\n"
           "  The location of this directory is defined in the emulator discovery file under the "
           "key grpc.jwks\n"
           "  Placing a valid JWK file with the name ${grpc.jwks}/my_filename.jwk will result in "
           "the JWK \n"
           "  being made available to the emulator. Note that `my_filename` can be an arbitrary "
           "file name.\n\n"
           "  The emulator will merge all discovered keys and write them to the file in the JWK "
           "format. The file\n"
           "  is defined in the emulator discovery file under the key grpc.jwk_active.\n\n"
           "  Developers can use this to make sure that the key they generated has been picked up "
           "by the emulator.\n\n"
           "  The location of the discovery file can be found in the directory is %s/avd/running\n"
           "  The file will be named `pid_%%d_info.ini` where %%d is the process id of the "
           "emulator. \n\n"
           "  Notes\n"
           "   - Tokens must be signed with one of the following algorithms: ES256, ES384, ES512, "
           "RS256, RS384, RS512\n"
...
static void help_grpc_allowlist(stralloc_t* out) {
    PRINTF("  A list that defines which gRPC methods can be accessed by whom.\n\n"
           "    <path_to_json_file> A json file describing the access rules.\n"
           "    see details in lib/emulator_access.json on how to specify rules.\n\n");
}

static void help_idle_grpc_timeout(stralloc_t* out) {
    PRINTF("  Terminates the emulator if there is no interaction on the gRPC endpoint.\n\n"
...
static void help_waterfall(stralloc_t* out) {
    PRINTF("  Mode in which to run the waterfall grpc endpoint..\n\n"
           "    \n\n"
...
static void help_grpc_ui(stralloc_t* out) {
    PRINTF("  Use the experimental gRPC based ui. This ui will make gRPC calls to accomplish "
           "tasks.\n\n");
}

static void help_share_vid(stralloc_t* out) {
    PRINTF("  Share the current video state in a shared memory region..\n\n"
...
static void help_packet_streamer_endpoint(stralloc_t* out) {
    PRINTF("  Use the packet stream at the given endpoint\n\n"
...
static void help_netsim_args(stralloc_t* out) {
    PRINTF("  Specify additional netsim arguments to pass to netsim instance.\n\n"
           ...
           "  Run `netsimd --help` to list all the available flags.\n"
           "  The netsimd binary should be located at the same folder as the emulator binary.\n\n");
}

static void help_onion(stralloc_t* out) {
    PRINTF("  use '-onion <file>' to specify a PNG image file that will be displayed on\n"
...
static void help_onion_alpha(stralloc_t* out) {
    PRINTF("  use '-onion-alpha <percent>' to change the translucency level of the onion\n"
...
static void help_onion_rotation(stralloc_t* out) {
    PRINTF("  use '-onion-rotation <rotation>' to change the rotation of the onion\n"
...
static void help_timezone(stralloc_t* out) {
    PRINTF("  by default, the emulator tries to detect your current timezone to report\n"
...
static void help_change_language(stralloc_t* out) {
    PRINTF("  Change the device's language to the language code specified.\n"
...
static void help_change_country(stralloc_t* out) {
    PRINTF("  Change the device's country to the country code specified.\n"
           "  The emulator will cold boot and then restart the framework again for\n"
           "  the setting to take effect.\n"
           "  Note that when doing so, the Quickboot snapshot will be overwritten\n"
           "  unless -no-snapshot-save is also issued.");
...
static void help_change_locale(stralloc_t* out) {
    PRINTF("  Change the device's locale to the locale code specified.\n"
           "  The emulator will cold boot and then restart the framework again for\n"
           "  the setting to take effect.\n"
           "  Note that when doing so, the Quickboot snapshot will be overwritten\n"
           "  unless -no-snapshot-save is also issued.");
...
static void help_dns_server(stralloc_t* out) {
    PRINTF("  by default, the emulator tries to detect the DNS servers you're using and\n"
...
static void help_net_tap(stralloc_t* out) {
    PRINTF("  by default, the emulator uses user mode networking which does address\n"
...
static void help_net_tap_script_up(stralloc_t* out) {
    PRINTF("  when using the -net-tap option it is sometimes useful to run a script when\n"
...
static void help_net_tap_script_down(stralloc_t* out) {
    PRINTF("  when using the -net-tap option it is sometimes useful to run a script when\n"
...
static void help_net_socket(stralloc_t* out) {
    PRINTF("  by default, the emulator's network device uses user mode networking which\n"
...
static void help_cpu_delay(stralloc_t* out) {
    PRINTF("  this option is purely experimental, probably doesn't work as you would\n"
...
static void help_no_boot_anim(stralloc_t* out) {
    PRINTF("  use '-no-boot-anim' to disable the boot animation (red bouncing ball) when\n"
...
#ifndef _WIN32
static void help_nand_limits(stralloc_t* out) {
    PRINTF("  use '-nand-limits <limits>' to enable a debugging feature that sends a\n"
...
static void help_bootchart(stralloc_t* out) {
    PRINTF("  some Android system images have a modified 'init' system that  integrates\n"
...
static void help_tcpdump(stralloc_t* out) {
    PRINTF("  use the -tcpdump <file> option to start capturing all network packets\n"
...
static void help_charmap(stralloc_t* out) {
    PRINTF("  use '-charmap <file>' to use key character map specified in that file.\n"
           "  <file> must be a full path to a kcm file, containing desired character map.\n\n");
}

static void help_prop(stralloc_t* out) {
    PRINTF("  use '-prop <name>=<value>' to set a boot-time system property.\n"
    ...
    PRINTF("  the corresponding system property will be set at boot time in the\n"
...
static void help_shared_net_id(stralloc_t* out) {
    PRINTF("  Normally, Android instances running in the emulator cannot talk to each other\n"
...
static void help_gpu(stralloc_t* out) {
    PRINTF("  Use -gpu <mode> to override the mode of hardware OpenGL ES emulation\n"
...
static void help_camera_back(stralloc_t* out) {
    PRINTF("  Use -camera-back <mode> to control emulation of a camera facing back.\n"
           "  Valid values for <mode> are:\n\n"
           ...
           "     videoplayback -> If the feature is enabled, camera will support playing back a "
           "video\n"
           "     none         -> camera emulation will be disabled\n\n");
}

static void help_camera_front(stralloc_t* out) {
    PRINTF("  Use -camera-front <mode> to control emulation of a camera facing front.\n"
           "  Valid values for <mode> are:\n\n"
...
static void help_webcam_list(stralloc_t* out) {
    PRINTF("  Use -webcam-list to list web cameras available for emulation.\n\n");
}

static void help_virtualscene_poster(stralloc_t* out) {
    PRINTF("  Use -virtualscene-poster <name>=<filename> to load an image as a poster in the\n"
...
static void help_screen(stralloc_t* out) {
    PRINTF("  Use -screen <mode> to set the emulated screen mode.\n"
           "  Valid values for <mode> are:\n\n"
...
static void help_selinux(stralloc_t* out) {
    PRINTF("  Use -selinux to control the SELinux enforcement mode.\n"
...
#ifdef __linux__
static void help_use_system_libs(stralloc_t* out) {
    PRINTF("  (DEPRECATED)\n\n"
...
static void help_stdouterr_file(stralloc_t* out) {
    PRINTF("  use '-stdouterr-file' to redirect stdout and stderr to it.\n\n");
}

static void help_no_qt(stralloc_t* out) {
    PRINTF("  use '-no-qt' to disable qt windowing system.\n\n");
}
#endif  // __linux__

static void help_unix_pipe(stralloc_t* out) {
    PRINTF("  Guest systems can directly connect to host Unix domain sockets using\n"
...
static void help_fixed_scale(stralloc_t* out) {
    PRINTF("  By default, we automatically scale the initial emulator window if it\n"
           "  doesn't fit into the screen. Use '-fixed-scale' to disable this.\n\n");
}

static void help_wait_for_debugger(stralloc_t* out) {
    PRINTF("  Request emulator process to pause at launch and wait for a debugger.\n"
           "  This option is useful for debugging it having a custom environment.\n\n");
}

static void help_cores(stralloc_t* out) {
    PRINTF("  Request the emulator to simulate <count> virtual CPU cores. This is\n"
...
static void help_detect_image_hang(stralloc_t* out) {
    PRINTF("  Enable the detection of overflowing goldfish event buffers.\n"
...
static void help_metrics_to_console(stralloc_t* out) {
    PRINTF("  Enable metrics reporting in the emulator and print the collected\n"
           "  data to stdout in JSON format.\n"
           "  Mutually exclusive with \"-metrics-to-file <file>\" and "
           "\"-metrics-collection\".\n\n");
}

static void help_metrics_collection(stralloc_t* out) {
    PRINTF("  Help make the emulator better by sending usage statistics\n"
           "  and crash reports to Google upon (graceful) emulator exit.\n"
           "  Mutually exclusive with \"-metrics-to-file <file>\" and "
           "\"-metrics-to-console\".\n\n");
}

static void help_metrics_to_file(stralloc_t* out) {
    PRINTF("  Enable metrics reporting in the emulator and write the collected\n"
...
static void help_no_metrics(stralloc_t* out) {
    PRINTF("  Disable metrics reporting in the emulator. Use this flag to bypass any\n"
...
static void help_feature(stralloc_t* out) {
    PRINTF("  Force-enable or disable an emulator feature by name. Also use the\n"
           "  ANDROID_EMULATOR_FEATURES environment variable for the same purpose.\n\n"
           "  E.g. \"-feature HAXM,-HVF -feature Wifi\"");
}

static void help_icc_profile(stralloc_t* out) {
    PRINTF("  ICC configuration file for SIM card\n\n");
}

static void help_sim_access_rules_file(stralloc_t* out) {
    PRINTF("  Textproto configuration file for overriding SIM access rules (see\n"
...
static void help_multidisplay(stralloc_t* out) {
    PRINTF("  Create displays besides the default android display. Format:\n"
...
static void help_google_maps_key(stralloc_t* out) {
    PRINTF("  User-provided API key to use with the Google maps GUI.\n"
           "  -google_maps_key <your_key>\n"
           "  Note that providing this key will override the one provided by Android Studio.\n\n");
}

static void help_no_location_ui(stralloc_t* out) {
    PRINTF("  Disables the location UI in the extended window.\n"
           "  -no-location-ui\n"
           "  This is useful if the location UI is interfering with your own location "
           "mocking.\n\n");
}
...
static void help_record_session(stralloc_t* out) {
    PRINTF("  Screen record the emulator session. Format:\n"
           "  -record-session <file>,<delay>[,<duration>]\n"
           "  Examples:\n"
           "  -record-session /tmp/tmp.webm,20 will start recording after 20 second delay until "
           "shutdown.\n"
           "  -record-session /tmp/tmp.webm,0,20 will start recording immediately for 20 "
           "seconds.\n\n");
}

static void help_legacy_fake_camera(stralloc_t* out) {
    PRINTF("  Emulates the fake camera by legacy camera HAL.\n\n");
}

static void help_camera_hq_edge(stralloc_t* out) {
    PRINTF("  Enable high quality edge processing for emulated camera.\n\n");
}

static void help_no_direct_adb(stralloc_t* out) {
    PRINTF("  Disables the internal ADB bridge and use the external adb bridge.\n");
}
...
static void help_wifi_tap(stralloc_t* out) {
    PRINTF("  by default, the emulator's virtio wifi device uses user mode networking which\n"
...
static void help_wifi_tap_script_up(stralloc_t* out) {
    PRINTF("  when using the -wifi-tap option it is sometimes useful to run a script when\n"
           "  TAP interface is enabled. Depending on the host system the TAP interface\n"
           "  might not be up until the emulator enables it. A common operation in such\n"
           "  a script is to add the TAP interface to a bridge so that it can be part of\n"
           "  the host network.\n"
...
static void help_wifi_tap_script_down(stralloc_t* out) {
    PRINTF("  when using the -wifi-tap option it is sometimes useful to run a script when\n"
           "  TAP interface is disabled. Depending on the host system the TAP interface\n"
           "  might be disabled when the emulator exits. A common operation in such\n"
           "  a script is to remove the TAP interface from a bridge that it was previously\n"
...
static void help_wifi_socket(stralloc_t* out) {
    PRINTF("  by default, the emulator's virtio wifi device uses user mode networking which\n"
...
static void help_vmnet_bridged(stralloc_t* out) {
    PRINTF(" Enable vmnet framework in bridged mode as the backend of tap netdev on MacOS and "
           "provide a host network interface as argument.\n\n");
}

static void help_vmnet_shared(stralloc_t* out) {
    PRINTF(" Enable vmnet framework in shared mode as the backend for tap network on MacOS\n\n");
}

static void help_vmnet_start_address(stralloc_t* out) {
    PRINTF("  If using vmnet in shared mode, starting IPv4 address to assign using DHCP.\n"
           "  Must be in RFC 1918 private IP range and specified together with vmnet_end_address "
           "and vmnet_subnet_mask.\n\n");
}

static void help_vmnet_end_address(stralloc_t* out) {
    PRINTF("  If using vmnet in shared mode, ending IPv4 address to assign using DHCP.\n"
           "  Must be in RFC 1918 private IP range and specified together with vmnet_start_address "
           "and vmnet_subnet_mask.\n\n");
}

static void help_vmnet_subnet_mask(stralloc_t* out) {
    PRINTF("  If using vmnet in shared mode, IPv4 netmask to assign using DHCP.\n"
           "  Must be in RFC 1918 private IP range and specified together with vmnet_start_address "
           "and vmnet_end_address.\n\n");
}

static void help_vmnet_isolated(stralloc_t* out) {
    PRINTF("Enable isolation for the interface. Ensures that vmnet interface is not able to "
           "communicate with any other vmnet interface.\n\n");
}
#endif
...
static void help_vmodule(stralloc_t* out) {
    PRINTF(" Argument is a comma-separated list of <module name>=<log level>.\n"
...
extern void android_help_list_options(stralloc_t* out) {
    ...
    PRINTF("\n");
    PRINTF("     %-*s  %s\n", maxwidth, "-verbose", "enable verbose output");
    PRINTF("     %-*s  %s\n", maxwidth, "-debug <tags>", "deprecated, use -vmodule");
    PRINTF("     %-*s  %s\n", maxwidth, "-debug-<tag>", "deprecated, use -vmodule");
    PRINTF("     %-*s  %s\n", maxwidth, "-debug-no-<tag>", "deprecated, use -vmodule");
    PRINTF("\n");
...
void android_help_main(stralloc_t* out) {
...
void android_help_all(stralloc_t* out) {
    const OptionHelp* oo;
    const TopicHelp* tt;
