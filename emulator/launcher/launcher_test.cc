...
class LauncherTest : public testing::Test {
  protected:
    void SetUp() override {
        ...
        ON_CALL(*mock_socket_factory, CreateServer(_, _, _, _))
                .WillByDefault(Return(std::make_shared<MockAsyncSocketServer>()));
...
TEST_F(LauncherTest, InterceptsSignalForSnapshot) {
    auto avd = CreateMockAvd();
    opts.snapshot = "test_snapshot";
    ...
    std::thread trigger = TriggerEmulatorScript(
            launched,
            [&]() {
                // Verification: SIGINT should NOT be forwarded to the process
