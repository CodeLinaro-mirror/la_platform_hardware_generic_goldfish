...
static std::vector<char> getXProperty(Display* disp, Window win, Atom xa_prop_type,
    ...
    if (XGetWindowProperty(disp, win, xa_prop_name, 0, 1024, False, xa_prop_type, &xa_ret_type,
...
static std::string getLinuxWindowManagerName() {
    ...
    std::vector<char> supWindowPtrAsChars = getXProperty(disp.get(), DefaultRootWindow(disp.get()),
                                                         XA_WINDOW, "_NET_SUPPORTING_WM_CHECK");
    if (supWindowPtrAsChars.empty()) {
        supWindowPtrAsChars = getXProperty(disp.get(), DefaultRootWindow(disp.get()), XA_CARDINAL,
    ...
    const auto supWindow = reinterpret_cast<const Window*>(supWindowPtrAsChars.data());

    std::vector<char> wmName = getXProperty(
            disp.get(), *supWindow, XInternAtom(disp.get(), "UTF8_STRING", False), "_NET_WM_NAME");
    if (wmName.empty()) {
        wmName = getXProperty(disp.get(), *supWindow, XA_STRING, "_NET_WM_NAME");
        if (wmName.empty()) {
