...
std::string vectorToHexString(const std::vector<uint8_t>& data) {
    ...
    for (int i = 0; i < data.size(); i++) {
        ss << "0x" << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(data[i]);
    }
    ss << "]";
    return ss.str();
}
...
bool PrintMinidumpProcess(const Options& options) {
    ...
    if (!options.symbol_paths.empty()) {
        symbol_supplier = absl::make_unique<SimpleSymbolSupplier>(options.symbol_paths);
    }
    ...
    FileReader reader;
    if (!reader.Open(base::FilePath(crashpad::ToolSupport::CommandLineArgumentToFilePathStringType(
...
std::unique_ptr<CrashReportDatabase> InitializeCrashDatabase() {
    ...
    for (int i = 0; !crashDatabase && i < 5; i++) {
        crashDatabase = crashpad::CrashReportDatabase::Initialize(database_path);
        if (!crashDatabase) {
...
static bool ListCrashReports(std::shared_ptr<CrashReportDatabase> crashDatabase) {
...
static bool DeleteCrashReports(std::shared_ptr<CrashReportDatabase> crashDatabase) {
    std::vector<crashpad::CrashReportDatabase::Report> reports;
    std::vector<crashpad::CrashReportDatabase::Report> pendingReports;
    crashDatabase->GetCompletedReports(&reports);
    crashDatabase->GetPendingReports(&pendingReports);
...
static bool UploadCrashReports(std::shared_ptr<CrashReportDatabase> crashDatabase) {
    std::vector<crashpad::CrashReportDatabase::Report> pendingReports;
    std::vector<crashpad::CrashReportDatabase::Report> reports;
    ...
    for (const auto& report : reports) {
        if (!report.uploaded) {
            ...
            android::crashreport::ProcessPendingReport(crashDatabase.get(), report);
