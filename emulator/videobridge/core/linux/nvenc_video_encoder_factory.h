...
/**
 * @class NvencVideoEncoderFactory
 * @brief WebRTC VideoEncoderFactory that advertises and creates NVENC hardware encoders.
 */
class NvencVideoEncoderFactory : public webrtc::VideoEncoderFactory {
  public:
    static std::unique_ptr<NvencVideoEncoderFactory> Create();
    static std::unique_ptr<NvencVideoEncoderFactory> CreateForTest(
            std::shared_ptr<NvencLoader> loader);

    explicit NvencVideoEncoderFactory(std::shared_ptr<NvencLoader> loader);
